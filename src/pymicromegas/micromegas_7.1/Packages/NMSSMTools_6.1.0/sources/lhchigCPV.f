...
***********************************************************************
      ...
      INTEGER I,I1,J,J1,NHGG1,NHGG2,NHGG3,NHGG4,NHGG5,NHGG6
      INTEGER JBAR,JBARbb,K,K1,K2,K3,NX,NA13,NC13,NC22
      ...
      DOUBLE PRECISION D1,D2,D3,MHcen,masstest,TBH
     ...
     . BRSUSY(5)
      ...
      DOUBLE PRECISION MHmin,MHmax,CBSM(5)
      DOUBLE PRECISION SIG(5,8),muH2,ggF8,ggF13,bbH8,bbH13,VBF13
      DOUBLE PRECISION SHGG,LHGG
...
c       I- Constraints from ggF/bb->H/A->tautau
      ...
      DO I=1,5
       CBSM(I)=1d0
      ENDDO
      D1=DDIM(dsqrt(MH0(1))/MHMAX,1d0)-DDIM(1d0,dsqrt(MH0(1))/MHMIN)
      D2=DDIM(dsqrt(MH0(2))/MHMAX,1d0)-DDIM(1d0,dsqrt(MH0(2))/MHMIN)
      D3=DDIM(dsqrt(MH0(3))/MHMAX,1d0)-DDIM(1d0,dsqrt(MH0(3))/MHMIN)
      IF(D1.EQ.0d0 .AND. D2.EQ.0d0)THEN
       ...
       ELSE
        CBSM(2)=0d0
       ENDIF
      ELSEIF(D2.EQ.0d0 .AND. D3.EQ.0d0)THEN
       IF(CV(2)**2.GT.CV(3)**2)THEN
        CBSM(2)=0d0
       ELSE
        CBSM(3)=0d0
       ENDIF
...
* ggF Signal cross section*BR:
          SIG1(I)=CBSM(I)*(CJ(I)**2+CJP(I)**2)*BRLL(I)*ggF8(MH(I))
...
* bbH Signal cross section*BR:
          SIGbb(I)=CBSM(I)*(CB(I)**2+CBP(I)**2)*BRLL(I)*bbH8(MH(I))
...
* ggF Signal cross section*BR:
          SIG1(I)=CBSM(I)*(CJ(I)**2+CJP(I)**2)*BRLL(I)*ggF13(MH(I))
...
* bbH Signal cross section*BR:
          SIGbb(I)=CBSM(I)*(CB(I)**2+CBP(I)**2)**2*BRLL(I)*bbH13(MH(I))
...
* New CMS 13 TeV limit (2405.18149):
        J=1
        DOWHILE(HGG4(J,1).LE.MH(I) .AND. J.LT.NHGG4)
          J=J+1
        ENDDO
        IF(J.GT.1 .AND. MH(I).LE.HGG4(NHGG4,1)) THEN
          LHGG=HGG4(J-1,2)+(MH(I)-HGG4(J-1,1))/
     .     (HGG4(J,1)-HGG4(J-1,1))*(HGG4(J,2)-HGG4(J-1,2))
        ...
        IF(I.LE.3)THEN
        J=1
        DOWHILE(HGG5(J,1).LE.MH(I) .AND. J.LT.NHGG5)
          J=J+1
        ENDDO
        IF(J.GT.1 .AND. MH(I).LE.HGG5(NHGG5,1)) THEN
          LHGG=HGG5(J-1,2)+(MH(I)-HGG5(J-1,1))/
     .     (HGG5(J,1)-HGG5(J-1,1))*(HGG5(J,2)-HGG5(J-1,2))
          SHGG=CV(I)**2*BRGG(I)*VBF13(MH(I))
          PROB(53)=PROB(53)+DDIM(SHGG/LHGG,1d0)
        ENDIF
        ENDIF
...
* New ATLAS 13 TeV limit (2407.07546):
        J=1
        DOWHILE(HGG6(J,1).LE.MH(I) .AND. J.LT.NHGG6)
          J=J+1
        ENDDO
        IF(J.GT.1 .AND. MH(I).LE.HGG6(NHGG6,1)) THEN
          LHGG=HGG6(J-1,2)+(MH(I)-HGG6(J-1,1))/
...
c      3) kappas(H_SM) and BR(H_SM -> BSM)
...
c        IX- Constraints from H->ZHSM

      CALL HEAVYH_ZHSM_CPV(PROB)
...
* normalized to SM XS
      ...
      I=1
      DOWHILE(MA.GE.HAATAUS4(1,1) .AND. HAATAUS4(I,1).LE.MA
     .        .AND. I.LT.NHAATAUS4)
       I=I+1
      ENDDO
...
* normalized to SM XS
      ...
      I=1
      DOWHILE(MA.GE.HAABS3(1,1) .AND. HAABS3(I,1).LE.MA
     .        .AND. I.LT.NHAABS3)
       I=I+1
      ENDDO
...
*     PROB(63) =/= 0: excluded 
      ...
      DOUBLE PRECISION PROB(*),S,SMAX,MA,MH
...
*     H_125 -> ZA, A -> hadrons from ATLAS: 2004.01678 + 2110.13673
*     PROB(75) =/= 0: excluded 
...
*   ATLAS 2110.13673, fig. 17b: Z -> ll + A -> mumu  and 15 < MA < 30 GeV
       ...
       IF(MA.GT.M2(1) .AND. MA.LE.M2(N2))THEN
        ...
        DO K=I+1,5
         MH=dsqrt(MH0(K))
         IF(MH.GE.MHMIN .AND. MH.LE.MHMAX)THEN
          S=S+(CJ(K)**2+CJP(K)**2)*gg13*BRHAZ(K,I)*BRMM(I)*.673d0
         ENDIF
        ENDDO
        PROB(75)=PROB(75)+DDIM(S/SMAX,1d0)
       ENDIF
...
*   CMS 2311.00130, fig. 7: Z -> ll + A -> gamgam  and 1 < MA < 30 GeV
       ...
       IF(MA.GT.M5(1) .AND. MA.LE.M5(N5))THEN
        S=0d0
        J=1
        DOWHILE(M5(J).LT.MA)
         J=J+1
        ENDDO
        SMAX=L5(J-1)+(MA-M5(J-1))/(M5(J)-M5(J-1))*(L5(J)-L5(J-1))
        DO K=I+1,5
         MH=dsqrt(MH0(K))
         IF(MH.GE.MHMIN .AND. MH.LE.MHMAX)THEN
          S=S+(CJ(K)**2+CJP(K)**2)*gg13*BRHAZ(K,I)*BRGG(I)*.673d0
         ENDIF
        ENDDO
        PROB(76)=PROB(76)+DDIM(S/SMAX,1d0)
       ENDIF
      ...
      SUBROUTINE HEAVYH_ZHSM_CPV(PROB)
...
*     PROB(79) =/= 0: excluded
      ...
      DOUBLE PRECISION MA,MH,SIGJBB,SIGBBB,SIGJLL,SIGBLL,L,ggF13,bbH13
      DOUBLE PRECISION MHC,XC(2,2),MH0(5),XH(5,5),MA2
      DOUBLE PRECISION CU(5),CUP(5),CD(5),CDP(5),CB(5),CBP(5),CJ(5),
     . CJP(5),CI(5),CG(5),CGP(5),CV(5),CZG(5),CZGP(5),CL(5),CLP(5)
      DOUBLE PRECISION WIDTH(5),HCWIDTH
      ...
      DO I=2,5
       ...
       DO J=1,4
        ...
        IF(DDIM(MH/MHMAX,1d0)-DDIM(1d0,MH/MHMIN).EQ.0d0)THEN
         SIGJBB=SIGJBB+(CJ(I)**2+CJP(I)**2)*ggF13(MA)*BRHAZ(I,J)*BRBB(J)
         SIGBBB=SIGBBB+(CB(I)**2+CBP(I)**2)*bbH13(MA)*BRHAZ(I,J)*BRBB(J)
         SIGJLL=SIGJLL+(CJ(I)**2+CJP(I)**2)*ggF13(MA)*BRHAZ(I,J)*BRLL(J)
         SIGBLL=SIGBLL+(CB(I)**2+CBP(I)**2)*bbH13(MA)*BRHAZ(I,J)*BRLL(J)
        ENDIF
       ENDDO
...
*     PROB(74) =/= 0: excluded
      ...
      INTEGER I,J,K,N,NHHH,NHHH2,NHHH3,NHHHT,NHHHT2,NHHHT3
...
*  CMS: 2403.16926, fig 28a (ggF -> H -> HSM+HSM)
     ...
     .     +DDIM(CV(I)**2*VBF13(MH)*SIG*(BRBB(J)/5.824d-1)**2/L,1d0)
          ...
          IF(K.GE.2 .AND. MH.LE.M2(N2)) THEN
           ...
           PROB(85)=PROB(85)
     .     +DDIM((CJ(I)**2+CJP(I)**2)*ggF13(MH)*SIG/L,1d0)
          ENDIF
