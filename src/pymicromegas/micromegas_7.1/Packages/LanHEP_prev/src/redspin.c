...
static void check_cc(List pl, Term m2)
{
    Term p, pp=0, pf=0;
   
    p=GetAtomProperty(CompoundArg1(ListFirst(pl)),PROP_TYPE);
    if(p && is_compound(p))
    {
    ...
    p=GetAtomProperty(CompoundArg1(ListFirst(ListTail(pl))),PROP_TYPE);
    if(p && is_compound(p))
    {
        if(CompoundName(p)==OPR_PARTICLE)
            pp=p;
    ...
    if(!pp || !pf)
        return;
    if(CompoundArg2(pf)!=NewInteger(4))
        return;
    if(CompoundArg1(pf)!=CompoundArg1(pp))
        return;
    if(CompoundArg1(pp)!=CompoundArg2(pp))
        return;
    p=CompoundArg1(m2);
    SetCompoundArg(p,1,NewInteger(IntegerValue(CompoundArg1(p))*2));
}
...
printf("im indices: ");	WriteTerm(i1); printf(" x "); WriteTerm(i2); puts("");
	...
	FreeAtomic(m);
       // WriteTerm(prt1);puts("");WriteTerm(m2);puts("");
       // WriteTerm(prt2);puts("");WriteTerm(m22);puts("");
        check_cc(prt1,m2);
        check_cc(prt2,m22);
