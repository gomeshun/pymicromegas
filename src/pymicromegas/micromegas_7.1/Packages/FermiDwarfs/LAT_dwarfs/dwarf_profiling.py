...
def scanner(scan_params, dwarf_data, ylist, spectrum, log10J, dJ, lnC, expo, dSph_tag, is_empirical):
    ...
    os.system('rm -f profiling_caseJ/sv_caseJ_'+ dSph_tag +'_'+str(n_scan)+'.dat')
    for sigv in ylist:
...
def scanner_stack(scan_params, ylist, dwarf_data, dlist, spectrum, expo, lnC, log10J, dJ, is_empirical):
    ...
    os.system('rm -f profiling_caseJ/sv_caseJ_' + "+".join(dlist) + '_' + str(n_scan) +'.dat')
    for sigv in ylist:
        ...
        def logL_tot_array(par):
            return sum(arrlogL2[k](par[k], dlist[k]) for k in range(len(dlist)))
        
        starts = [18.0             for _ in dlist]
        errors = [2.0              for _ in dlist]
        limits = [(10.0, 22.0)     for _ in dlist]
                
        m = Minuit(
            logL_tot_array, starts,
        )
...
def scanner_stackB(scan_params, ylist, dwarf_data, dlist, spectrum, log10J, dJ, ratios, expo, lnC, lnCV0, xD, xV, is_empirical):
    ...
    Nbins=6
    Ngammabins = dNdE_integrated
    os.system('rm -f profiling_caseJB/sv_caseJB_' + "+".join(dlist) + '_' + str(n_scan) + '.dat')
    dwarfs_emp = ["Draco", "Carina", "Fornax", "LeoI", "LeoII", "Sculptor", "Sextans", "UrsaMinor"]
    ...
    for sigv in ylist:
        ...
        def logL_tot(par):
            """
            theta: numpy array [J_0, B_0, J_1, B_1, ...]
            Returns the summed log-likelihood over all sources.
            """
            total = 0.0
            for k, tag in enumerate(dlist):
                Jk = par[2*k]
                Bk = par[2*k+1]
                total += arrlogL2[k](Jk, Bk, tag)
            return total
