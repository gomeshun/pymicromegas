...
int fillCutArray(void)
{
  linelist ln;
  int lineNum=0;
  char cutStr[STRSIZ], minStr[STRSIZ], maxStr[STRSIZ],auxStr[40];
  int minOn,maxOn,aux;
  char fieldName[50];
...
/*============ Parameter ===========*/
    sprintf(fieldName,"Wrong field 'Parameter':%s",cutStr);
...
/* =========== fill array ==========*/
    ...
    {  k=addcut(aux,cutStr,minOn,maxOn,min_,max_);
       ...
       { case 2: strcpy(fieldName,"Too many cuts ");         goto errorExit;
         ...
         case 4: sprintf(fieldName,"Error in parameter:%s:",cutStr); goto errorExit;
       }  
    }   
  } 
  return 0;
