...
static void write_event_cap(void) 
{
  ...
  if(nin_int==1) fprintf(events_,"  P1_3=%E\n",inPD);
  else
...
int readVegasGrid(FILE * f)
{
  ...
  if(ndim && ndmx)
  { 
    veg_Ptr=vegas_init(ndim,vegasSQME,ndmx);
...
double vegasSQME(double *x, double wgt)
{
    double ret_val=0.;
    int err=0,nd,i;
    double factor_0;
...
int runVegas(void)
{
    ...
    if(!veg_Ptr) veg_Ptr=vegas_init(i,vegasSQME,50);     
...
/*          integral.freeze=0; */
        ...
        case 8: if(!integral.freeze || mess_y_n(15,15,"The information for Event Generator will be lost\n OK?"))  
                { int ndim=veg_Ptr->dim;
                  ...
                  veg_Ptr=vegas_init(ndim,vegasSQME,50);
                  messanykey(57,11,"OK");
                }   
                break;
        case 9: 
