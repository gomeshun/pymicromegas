...
static void  gaxes(char* upstr, char* xstr, int N, char**Y)
{
   ...
   { double zmin,zmax,zmin1,zmax1;
     ...
     {  tg_settextjustify(LeftText,CenterText);
        goto_xy(10,2);
        for(i=0;i<N;i++)   
        {  if(colList[i]<bkcolor) scrcolor(colList[i],bkcolor); else scrcolor(colList[i+1],bkcolor);
           print("%s",Y[i]);
           if(i<N-1) print("; ");
        }       
     }
...
#define MESS "Esc - exit; Mouse - position; Other keys - menu"
   ...
   { gminmax(f[i],ff[i],Dim[i], &ymin,&ymax);
     if(!isfinite(grafminmax.ymin))  grafminmax.ymin=ymin; else if(grafminmax.ymin>ymin) grafminmax.ymin=ymin;
     if(!isfinite(grafminmax.ymax))  grafminmax.ymax=ymax; else if(grafminmax.ymax<ymax) grafminmax.ymax=ymax;   
   }
...
REDRAW:
   ...
   for(i=0;i<N;i++)
   {   if(colList[i]<bkcolor) fColor=colList[i]; else fColor=colList[i+1];
...
//     if(y<=yMin||y>=yMax) continue;
     ...
     { 
       ...
       {  double x0,h,al;
          ...
          if(colList[k]<bkcolor) scrcolor(colList[k],bkcolor); else scrcolor(colList[k+1],bkcolor);
