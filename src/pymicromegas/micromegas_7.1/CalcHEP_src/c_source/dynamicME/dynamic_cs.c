...
int  getNumCoeff(lVert*vv, double * coeff)
{
   ...
   { 
     ...
     varNames[*currentVarPtr], vv->varNames[i]);  
...
double lGGqcd(double Q,char*h)
{  
printf("lGGqcd\n");
   int i; 
   for(i=0;i<nModelParticles;i++) if(strcmp(ModelPrtcls[i].name,h)==0) break;
   if(i==nModelParticles) { printf(" %s - no such particles\n",h);   return 0;}
   if( ModelPrtcls[i].spin2!=0 ||  ModelPrtcls[i].cdim!=1 ||  !ModelPrtcls[i].selfC)  
   { printf(" %s - is  not a Higgs-like particle \n",h);   return 0;}
   ...
   { char *p=pdg2name(i+4);
     ...
     { double m=pMass(p); 
       if(m) 
       { char*a = antiParticle(p);   
         lVert *xxh=getVertex(a,p,h,NULL);   
         if(xxh)
         for(int k=0;k<xxh->nTerms;k++)  if(strcmp(xxh->SymbVert[k],"1")==0)
         { double coeff[10];
   ...
   double C1[3],C2[3];
   hWidthCoeff2(Q,C1,C2 ); 
   double c1=0,c2=0;
   
   for(int i=0;i<3;i++) { c1+=C1[i]*v[i]; c2+=C2[i]*v[i];}
   if(fabs(c1)>fabs(c2)) return c1; else return c2;            
}
