...
char * antiParticle(const char*name)
{  
   int i; 
   for(i=0;i<nModelParticles;i++)
...
int qNumbers(const char*pname, int *spin2, int * charge3, int * cdim)
{
  int n=pTabPos(pname);
  int sign=1;
  int pdg;
...
double  findValW(const char*name)
{ double val;
