...
static int wBest=15,wMin=4,wMax=35;
...
int crt0_keypressed(void)
{
  ...
  if( XCheckMaskEvent(display,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|
		...
		case KeyPress:
		    ...
		    XLookupString(&report.xkey,&c,1,&keysym,NULL);
                    if(CtrlPressed)
                    {
                       ...
                       if( ( keysym==XK_plus || keysym==XK_equal)&& wBest<wMax)     
                       {  wBest++;
