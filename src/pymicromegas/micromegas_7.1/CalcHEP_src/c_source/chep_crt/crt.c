...
int  start1(char * window_name,char * icon_file,char * ini_file, void(*xw_errorExit)(void))   /* by A.Pukhov */
{  int err=0;
   if(blind==2) printf("\n\"");
   if(blind==1) return 0;
   char*iniF=NULL;
   ...
   if(ini_file) 
   {
      char *buff=malloc(strlen(ini_file)+2);
      strcpy(buff,ini_file); 
      for(char *a=buff;a;)
      { 
         char*b=strchr(buff,';'); if(b) b[0]=0;
         trim(a);
         if(strlen(a)==0) break;
         if(!access(a,R_OK)) { iniF=malloc(strlen(a)+1); strcpy(iniF,a); break;}
         if(b)a=b+1; else break;
      }   
      free(buff);
   }
   
   if(iniF==NULL) { iniF=malloc(strlen(rootDir)+20);  sprintf(iniF,"%s/calchep.ini",rootDir); }

   err=crt0_start(window_name,icon_file,iniF,&nColors,xw_errorExit);
   free(iniF);
