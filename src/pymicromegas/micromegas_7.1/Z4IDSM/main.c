...
{ int fast=0;
  ...
  printf("Testing  of 2 WIMPs scenario\n");   
  Tend=1E-3; 
  ...
  { 
     printf("  ......   not realized\n Testing FIMP+WIMP scenario.\n");
     double TR=1E4;  
     Tend=pMass(CDM[2])/20; 
     char fimp[20];  strcpy(fimp,CDM[1]);            
     toFeebleList(fimp); 
     sortOddParticles(NULL);
     double omegaFi=darkOmegaFi(TR,fimp, &err);
     double Y[2]={ omegaFi/Mcdm/2.742E8, YdmNEq(TR,"2")};  
//     Y[0]=0;
     TR=Tend;   
