...
static double (*SigmaExp)(double M)=XENON1T;
...
static double maxDev(int prn) 
{
  ...
  { double cs=SigmaExp(Mcdm)*1E36;
    ...
    double nEvents=Exposure*simpson_arg((funcArg)recoilSignal,dNdE,Etab[0]+0.001, Emax, 1E-3,NULL);
...
//displayPlot("Xenon/Panda/LZ","M", 6,10000, 1,3,"Xenon",0, XENON1T, NULL
displayPlot("Xenon/Panda/LZ","M", 10,300, 0,3,"Xenon",0, XENON1T, NULL
...
//  improved acceptance 
   ...
   displayPlot("Xenon1T SI. 90\% and 3 sigma ","M[GeV]", mMin,mMax,1,4 
                                                 ,"Xenon 1T(exp.)  90\%",0,XENON1T,NULL          
...
//   double f= DD_factorCS(DarkSide_2018,0.1,Maxwell, 1E-12,1E-12 ,0,0,&expName);
//printf("f=%E\n",f);
f=525;  
printf("f=%E\n",f);
  double pval=DD_pvalCS(DarkSide_2018, Maxwell, 1E-12*f, 1E-12*f, 0, 0,&expName);
  printf("pval=%E\n", pval);
f=1E-15;
printf("\nf=%E\n",f);
  pval=DD_pvalCS(DarkSide_2018, Maxwell, 1E-12*f, 1E-12*f, 0, 0,&expName);
  printf("pval=%E\n", pval);  
//  printf("plr2pval(1)=%E\n", plr2pval(1));
//  printf("plr2pval(1.01)=%E\n", plr2pval(1.01));
...
DS_neStart=4;
  ...
  displayPlot("DS_50 90\% exclusion (Bin.Fluct.)","M[GeV]", mMin,mMax,0,4
                                                        ,"DS_50_Bin",0,DS50,NULL
                                                        ,"DS_50_noBin",0,DS50_noB,NULL            
...
displayPlot("CRESST-III  90% exclusion","M[GeV]", mMin,mMax,1,2
                 ,"CRESST_III  SI   ",0, CRESST_III,NULL
                 ,"CRESST_III SDn",0, CRESST_III_SDn,NULL
           );      
...
extern double  CRESST_III_SDn_(double);

  displayPlot("CRESST-Spin Dependent(N) 90%% exclusion","M[GeV]", mMin,mMax,1,2
                 ,"CRESST_III_SDn",0, CRESST_III_SDn,NULL
//                 ,"CRESST_SD_OLD",0, CRESST_III_SDn_,NULL
...
//  printf("M=%.2E   diffSI=%6.3f   diffSD=%6.3f \n", X[i],  resSI/CRESST_III(X[i])-1, resSD/CRESST_III_SDn(X[i])-1); 
}
...
//     spinDepFlag=EFT;
  ...
  displayPlot("Xenon1T-Spin Dependent   90%% exclusion","M[GeV]", mMin,mMax,1,2
                 ,"SD_p",0, XENON1T_SDp,NULL
                 ,"SD_n",0, XENON1T_SDn,NULL
                                                );  
