...
//if(PRN) { printf("T=%.2E p=%.2E *df=%.2E Cprod=%.2E \n", T,p,*df,Cprod(p,T,par->kL,LasymmS(T)*(2*M_PI*M_PI/45.*hEff(T))));}     
...
if(i==394 && exp(lnTgrid[n-1]) <5.22E-02+0.001)  
{  printf("ip=%d T in [%.2E, %.2E]\n", i, exp(lnTgrid[n-1]), exp(lnTgrid[n]));  PRN=1;
...
//           printf("ip=-%d T in [%.2E, %.2E]\n", i, exp(lnTgrid[n-1]), exp(lnTgrid[n]));
