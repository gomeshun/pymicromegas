...
static double intEe(double E, double *tab) { return (E+5E-4)*SpectdNdE(E,tab)*fe(E*1E9);}
...
{ int err;
  ...
  r+=simpson_arg((funcArg)intEA,tab,5E-6,Mdm, 1E-3,&err)/(2*Mdm);
//if(err) { sprintf(txt,"A,%d %E\n",pdg,Mdm);  displayPlot(txt,"E",5E-6,Mdm,1,1,"A",0,intEA,tab);}
  basicSpectra(Mdm, pdg, 1, tab);
  r+=simpson_arg((funcArg)intEe,tab,5E-6,Mdm, 1E-3,&err)/Mdm;
...
int main(void)
{
  displayPlot("f_eff, Fig.3 1506.03811","E[eV]",5E3,1E13,1,2,"electron",0,fe,NULL
                                                            ,"photon",  0,fA,NULL);

  SpectraFlag=2;  // PPPC according to 1506.03811
 ...
 //  Indeed for all spectra generators the fraction of energy that goes to photons and electrons slightly decreases when the  DM mass increases.
  ...
  m=1E5;
   ...
   displayPlot("xdNdx (photons&electrons- PPPC)","E", 1E-8 ,1,1,2,"M=1E3",0,xInterp,t1,"M=1E5",0, xInterp,t2);
   displayPlot("xdNdx (photons&electrons- PPPC)","E", 1E-8 ,1,0,2,"M=1E3",0,xInterp,t1,"M=1E5",0, xInterp,t2); 

SpectraFlag=0;

   m=1;
   basicSpectra(m,13, 1, te);
   displayPlot("int e","E", 5E-6, 1000,1,1,"integrand", 0, intEe,te);
