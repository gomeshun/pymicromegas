...
9510408     free energy
 ...
 de/dT = T ds/dT                              gEff(T)-hEff(T) =  T*dhEff(T)/dT /3 - T*dgEff(T)/dT/4
 ...
 I/T^4 = T d(p(T)/T^4)/dT
          ...
          Hadrons:
static  double hadrons(double T, double (* F)(double,double,int), int mode ); 
...
contributions are contribute. Function F, for example, can be 
       F(M/T,fermion) = h1eff_(M/T, fermion)  - the contribution of one 
...
static double h1eff_(double mu,int eta){ return h1eff(mu,0,eta);}
static double g1eff_(double mu,int eta){ return g1eff(mu,0,eta);}
...
static double press1divT4(double t, int eta) { return M_PI*M_PI/90*(4*h1eff_(t,eta)-3*g1eff_(t,eta));}    // pressure*T^4  for 1 degree of freedom,  t=M/T
static double I1T4(double t, int eta){ return 2*M_PI*M_PI/15*(g1eff_(t,eta)-h1eff_(t,eta));}            // e-m trace *T^4 for 1 degree of freedom,  t=M/T
...
{  return 45/(2*M_PI*M_PI)*(IdivT4(T)+4*pdivT4(T))+     // Nf=3 lattice calculation
  (
     2 + 6*h1eff_(80/T,1) + 3*h1eff_(91/T,1) + h1eff_(125/T,1)        //  bosons
     + 7./8.*2*3                                                   //  neutrinos
     + 4*( h1eff_(5E-4/T,-1) + h1eff_(0.1/T,-1) + h1eff_(1.7/T,-1))   //  charged leptons
     + 12*(h1eff_(Mc/T,-1) +h1eff_(5/T,-1) + h1eff_(175/T,-1) )       //  heavy quarks
  );
...
static double hEffI(double T) // ideal gas approximation
{
  ...
  double sum= 2 +6*h1eff_(80/T,1) + 3*h1eff_(90/T,1) + h1eff_(125/T,1)              // bosons
                +16                                                              // gluon
                +7./8.*2*3                                                       // neutrinos
                +7./8.*2*2 +4*h1eff_(0.106/T,-1)  + 4*h1eff_(1.7/T,-1)             // charged leptons
                +12*( 2*7./8.+  h1eff_(0.101/T,-1)+ h1eff_(1.4/T,-1) +h1eff_(5/T,-1) + h1eff_(175/T,-1)) // quarks
            ;
  return sum;
}
...
static double hEffIhadrons(double T) // ideal gas approximation
{
  ...
  double sum=  2 + 6*h1eff_(80/T,1) + 3*h1eff_(90/T,1) + h1eff_(125/T,1)          // bosons without gluon
              +7./8.*2*3                                                       // neutrinos
              +7./8.*2*2 +4*h1eff_(0.106/T,-1) + 4*h1eff_(1.7/T,-1)              // charged leptons
              + hadrons(T, h1eff_,1)                                            // hadrons
            ;
  return sum;
}
