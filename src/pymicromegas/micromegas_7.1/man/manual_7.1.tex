...
\newcommand{\tcv}[1]{{\color{violet} #1}}
\newcommand{\tcg}[1]{{\color{olive} #1}}
...
\verb|    https://zenodo.org/records/17597395|\\
...
This should create the directory \verb|micromegas_|\VERSION\verb|/| which occupies about 100Mb of disk space. You will need more disk space after compilation of
specific models and generation of matrix elements. More information can also be found at\\
\verb|     http://lapth.cnrs.fr/micromegas|\\
...
\section{The Inert Doublet Model : an example of BSM presentation in \texttt{micrOMEGAs}}

The predictive power of \texttt{micrOMEGAs} rests on its tight coupling with the matrix-element generator \texttt{CalcHEP}~\cite{Belyaev:2012qa}.  
Every physical quantity computed by \texttt{micrOMEGAs} -- from decay widths to relic density integrals -- ultimately originates from symbolic expressions generated in \texttt{CalcHEP}.  
For this reason, any extension of the Standard Model must first be expressed in the \texttt{CalcHEP} model format before it can be used in numerical studies.

A minimal model definition consists of four interrelated files,
\[
\texttt{vars1.mdl}, \quad \texttt{func1.mdl}, \quad \texttt{prtcls1.mdl}, \quad \texttt{lgrng1.mdl},
\]
which together encode the entire theoretical structure:
the list of independent parameters, the relations among derived quantities, the spectrum of particles, and the set of interaction vertices.
Although the syntax is deliberately simple, the logical separation between these files reflects the conceptual hierarchy of a field theory:  
input parameters determine all derived quantities, which in turn fix the particle properties and the Feynman rules that govern dynamics.

To illustrate how these ingredients cooperate, we use the \emph{Inert Doublet Model} (IDM) as an example.  
The IDM provides one of the simplest frameworks in which an exact discrete symmetry stabilises a scalar dark matter particle, yet it retains close contact with Standard Model phenomenology.

\subsection{Theoretical structure of the IDM}

The model introduces a second $SU(2)$ doublet $H_2$ in addition to the Standard Model Higgs doublet $H_1$.  
Its Lagrangian is
\begin{equation}
\label{L_IDM}
  L = (SM\;\text{terms}) + D^{\mu}H_2^*D_{\mu}H_2 
 - \mu^2 H_2^2 - \lambda_2 H_2^4 - \lambda_3 H_1^2H_2^2
- \lambda_4 |H_1^*H_2|^2 - \lambda_5 \text{Re}\!\left[(H_1^*H_2)^2\right],
\end{equation}
with
\begin{equation}
H_1 =
...
H_2 =
\begin{pmatrix}
\tilde H^+ \\ (\tilde X + i\tilde H_3)/\sqrt{2}
\end{pmatrix}.
\end{equation}
Only even powers of $H_2$ appear, ensuring an exact $Z_2$ symmetry $H_2 \to -H_2$.
This prevents mixing with $H_1$ and guarantees the stability of the lightest $Z_2$--odd state -- identified in \texttt{micrOMEGAs} as the dark matter particle.
By convention, all odd fields must have names beginning with the tilde character `\verb|~|', which allows the code to recognise them automatically.
See  Section \ref{DiscretSym} for details. 

\subsection{Translation into \texttt{CalcHEP} syntax}

The theoretical ingredients discussed above are distributed across the four \texttt{CalcHEP} model files, each fulfilling a distinct role in the numerical implementation.  
Tables \ref{tab:variablesA}--\ref{tab:FeynRules} illustrate this correspondence step by step: the independent and derived parameters in \texttt{vars1.mdl} and \texttt{func1.mdl}, followed by the particle content and interaction vertices specified in \texttt{prtcls1.mdl} and \texttt{lgrng1.mdl}.

\paragraph{Model parameters.}
The model definition starts with the list of input and derived parameters.  
The independent quantities are defined in the file \texttt{vars1.mdl} and are shown in Table~\ref{tab:variablesA}.  
They represent the fundamental inputs supplied by the user or fixed to Standard Model reference values.  
In the IDM, these include the electromagnetic coupling, the weak mixing angle, the $Z$--boson mass, and the masses of the inert scalars.

\begin{table}[htbp]
\centering
\begin{tabular}{l|l|l}
\textbf{Name} & \textbf{Value} & \textbf{Comment} \\
\hline
...
... & ...     & ...
\end{tabular}
\caption{Independent input parameters defined in \texttt{vars1.mdl}.}
\label{tab:variablesA}
\end{table}

The dependent or constrained quantities are then specified in \texttt{func1.mdl} and summarised in Table~\ref{tab:variablesB}.  
These relations express the derived parameters -- such as the $W$--boson mass or the quartic couplings $\lambda_i$ -- in terms of the independent inputs.  
This separation between inputs and functions ensures that parameter scans or theoretical consistency checks can be performed automatically without rewriting algebraic expressions in the model files.
...
CW & sqrt(1-SW\pw2)\\
MW & MZ*CW \\
mu2 & MHX\pw2 - LaL*(2*MW/EE*SW)\pw2 \\
la3 & 2*(MHC\pw2 - mu2)/(2*MW/EE*SW)\pw2 \\
la5 & (MHX\pw2 - MH3\pw2)/(2*MW/EE*SW)\pw2 \\
... & ...
\end{tabular}
\caption{Derived parameters and algebraic relations defined in \texttt{func1.mdl}.}
\label{tab:variablesB}
\end{table}

\paragraph{Particle content.}
The full particle inventory is given in Table~\ref{tab:particles}, which corresponds to the file \texttt{prtcls1.mdl}.  
Each row specifies the quantum numbers and numerical identifiers of a physical state: the PDG code ensures interoperability with external programs, while the keyword \texttt{!wX} marks widths that \texttt{micrOMEGAs} should compute automatically.  
The three new inert scalars -- $\sim$X,  $\sim$H3, and $\sim$H+ -- are explicitly introduced here alongside the Standard Model fields. 
Note that the symbol \verb|!| before the width means that it will be computed automatically by CalcHEP from the interaction vertices.
Simbol {\tt G} in the {\tt aux} column means that the particles will be treated in Feynman gauge and contribution of   unphysical components of vector field has to be compensated  by Faddeev-Popov and Goldstone ghosts.
...
odd Higgs & $\sim$H3 & $\sim$H3 & 36 & 0 & MH3 & !wH3 & 1 &  \\
Charged Higgs & $\sim$H+ & $\sim$H- & 37 & 0 & MHC & !wHC & 1 &  \\
second Higgs & $\sim$X & $\sim$X & 35 & 0 & MHX & !wHX & 1 &  \\
... & ... & ... & ... & ... & ... & ... & ... & ...
\end{tabular}
}
\caption{Particle content of the IDM as implemented in \texttt{prtcls1.mdl}.  The tilde identifies $Z_2$–odd states recognised by \texttt{micrOMEGAs} as dark-sector fields.}
\label{tab:particles}
\end{table}


\paragraph{Interaction vertices.}
The couplings that drive all physical processes are encoded in \texttt{lgrng1.mdl}, shown in Table~\ref{tab:FeynRules}.  
Each line corresponds to a term of the Lagrangian, expressed symbolically in the \texttt{CalcHEP} notation.  
Dirac matrices are denoted by \texttt{G}, while four-momenta are referenced as \texttt{m1.p2}, etc.  
For example, the entry involving \{h,$\sim$ X,$\sim$X\} represents the Higgs--portal interaction that controls both the annihilation rate of dark matter and its spin-independent scattering cross section.
...
\hline
A & W+ & W- &   & -EE & m3.p2*m1.m2 - m1.p2*m2.m3 - ... \\
A & $\sim$H+ & $\sim$H- &   & EE  & m1.p3 - m1.p2 \\
h & $\sim$X & $\sim$X &   & -2*MW*SW/EE & la3 + la4 + la5 \\
Z & Z & $\sim$X & $\sim$X & EE\pw2(2*CW\pw2*SW\pw2) & m1.m2 \\
... & ... & ... & ... & ... & ...
\end{tabular}
}
\caption{Selected interaction vertices from \texttt{lgrng1.mdl}.  
These entries define the Feynman rules used in all subsequent cross-section calculations.}
\label{tab:FeynRules}
\end{table}

See more details about implementation of BSM models in micrOMEGAs and generation of matrix elements in Sections \ref{sec:newmodel}, \ref{cross_section}
...
Information about all particles is stored in array {\tt ModelPrtcls} which contains {\tt nModelParticles}  elements.
Each element of array is a {\tt struct} which contains the fields: \\
\hspace{1cm}{\it char*} name; // name of particle\\
{\it char*} aname; // name of anti-particle\\
{\it int } NPDG ; // PDG MC code\\
{ \it char*} mass; // name of mass   identifier \\
{\it  char*}  width; // name of width identifier \\
{\it int }  spin2; // double spin \\
{\it int }  cdim;   // dimension of color representation \\ 
{\tt int } g; // number of degrees of freedom  \\
...
\section { Thermodynamics of the Universe.}
...
\end{equation}
  These functions play a key role in the calculation of the  DM relic density: $\rho_R(T)$ defines the Hubble rate $H$, Eq.\ref{Hubble}, and $s(T)$ allows to
find the relation between the evolution of time and temperature after imposing entropy conservation
\begin{eqnarray}
\label{entropyCons}
     \frac{ds}{dt}&=&-3Hs\\
...
micrOMEGAs contains several files with tabulated data for {\tt gEff} and {\tt gEff}. They are stored  in the folder {\tt Data/hgEff/}.  
By default the tables for  $h_{eff}, g_{eff}$ correspond to the ones in Ref.~\cite{Drees:2015exa} and can be found in the file \verb|Data/hgEff/DHS.thg|.
These default tables can be changed using  \\
%
\noindent$\bullet$ \verb|loadHeffGeff(char*fname)|\\
that  looks for  the file {\it fname}   located in the directory {\tt Data/hgEff} or in the current directory. This file should 
...
The  directory  {\tt Data/hgEff}  also contains solutions described in Ref.~\cite{Laine:2015kra}, {\tt LM.thg},  and in Ref.~ \cite{Hindmarsh:2005ix},  {\tt HP\_B.thg, HP\_C.thg},  as well as the tables used in DarkSUSY, {\tt GG.thg}. The latter  was  used as default in  previous versions of
...
\\  
%
See Section \ref{sec:slowInflaton} for non-standard cosmology.

\subsection{Modification of {\tt geff,heff}}

 
Note that entropy conservation follows from \footnote{ This
equation is not valid for $T\le 10$ MeV, where photons and neutrinos have
different temperatures, or  in case of lepton asymmetry.} 
\begin{equation}
    \frac{d\rho}{dT} = T\frac{d s}{dT}  
\end{equation}
This relation is however not  reproduced in the tables {\tt LM.thg} and {\tt DHS.thg} in the region of quark deconfinement. An improved solution that imposes entropy conservation is
provided  in the files {\tt LMi.thg} and {\tt DHSi.thg}. The correction can be found in  the code {\tt mdlIndep/improveGeff.c } which outputs a comparison between the two sets of tables. 


New functions allow the user to include the contribution of new particles beyond the SM. \\
\noindent$\bullet$~\verb|h1eff(M/T,mu/T, eta)| \\
\noindent$\bullet$~\verb|g1eff(M/T,mu/T, eta)| \\
return the contribution of a single internal degree of freedom of a particle with mass \texttt{M} and chemical potential \texttt{mu}
to the effective entropy (\texttt{hEff}) and energy (\texttt{gEff}) density functions.  
The argument \texttt{eta} specifies the particle statistics: \texttt{eta = 1} for bosons and \texttt{eta = -1} for fermions.  
To obtain the total contribution of a new particle species, the value returned by \verb|h1eff| or \verb|g1eff| must be multiplied by its internal 
degree of freedom $g_i$. By default, \texttt{micrOMEGAs} includes only Standard Model particles in 
\texttt{hEff} and \texttt{gEff}; these functions allow the user to add the contribution of additional 
BSM particles present in the thermal bath to the total effective degrees of freedom entering the computation 
of the Hubble rate and entropy density. To do this the user can create a new table in the format of the existing ones in the \verb|Data/hgeff| directory and then use \verb|loadHeffGeff("filename")|. The chemical potential is necessary  to treat models with lepton asymmetry, otherwise the user can set $\mu/T=0$.
 
The functions \\
\noindent$\bullet$ \verb|p1eff(M/T,mu/T,eta)|\\
\noindent$\bullet$ \verb|n1eff(M/T,mu/T,eta)|\\
can be used to calculate the  pressure and number density of one degree of freedom
\begin{eqnarray}
p&=& \frac{\pi^2}{90}  {\rm p1eff}(M/T,\mu/T,\eta)T^4\\
n&=&  {\rm n1eff}(M/T,\mu/T,\eta)T^3
\end{eqnarray}
These functions are normalized so that 
\begin{eqnarray}
 {\rm h1eff(0,0,1)}&=&{\rm g1eff}(0,0,1)={\rm p1eff}(0,0,1)=1\\
{\rm  n1eff}(0,0,1)&=&\frac{\zeta(3)}{\pi^2}
\end{eqnarray}  



\section{Relic density calculation}
\subsection{Switches and auxiliary routines}
\label{Switches}

 micrOMEGAs can treat three different  scenarios  for the calculation of the  relic density :
{\it freeze-out}, {\it freeze-in} and modified cosmology denoted as  {\it inflaton}. The scenario can be selected automatically by specifying a flag in the input file :
\begin{verbatim}
#! FreezeOut
#! FreezeIn 
#! Inflaton 
...
\noindent$\bullet$ \verb|improveCrossSection( n1,n2,n3,n4, Pcm,T, &cs)|\\
...
[${\rm GeV}^{-2}$].  The cross-section can be temperature dependent, 
\verb|T| is the temperature in [GeV]. This function is called just after the calculation
...
\subsubsection{darkOmegaExt} 

\noindent
$\bullet$ \verb|darkOmegaExt(&Xf, vs_a, vs_s, vs_3, vs_4,&err)|\\
calculates the dark matter relic density $\Omega h^2$  
using annihilation cross sections of two odd particles. The annihilation cross-sections are provided by external 
functions  {\tt vs\_a, vs\_s, vs\_3,vs\_4}  
which return vSigma in [$pb\cdot c$] units as a function of temperature. 

Here  \verb|vs_a|  is the  annihilation cross section  
into two SM particles  as in Eq.\ref{vSigmaA},  while  \verb|vs_s| is the
semi-annihilation cross section   with production of 
one outgoing odd particle, Eq.\ref{vSigmaS}. {\tt vs\_3} and {\tt vs\_4} correspond to the
annihilation cross section  into  3 and 4 odd particles, these
 can be relevant for  strongly interacting DM
\footnote{ Processes with 2 outgoing odd particles  are omitted because they do not change the number of odd particles.}. {\tt vs\_3(T)} and {\tt vs\_4(T)}  must  include an additional
factor of $ exp(Mcdm/T)$ and $ exp(2Mcdm/T)$ respectively. These factors arise because the main processes are  the reverse ones ( $3\to2$ and $4\to2$) which are taken into account via the detailed balance equation. These contributions can be essential at low temperatures when the direct ones becomes zero because of Boltzmann suppression~\cite{Belanger:2026ctm}.
 Note that some input functions may not be relevant, in this case  the
user should replace them with  {\tt NULL}. 
 

\verb|darkOmegaExt| solves the following abundance equation
\begin{eqnarray}
\nonumber
 \frac{dY}{dT}&=&  - v\sigma_a(Y^2-Y_{eq}^2)
        -\frac{1}{2} v\sigma_s (Y^2-Y_{eq}Y)\\
\label{equExt}
        &+&\frac{1}{2} v\sigma_3 Y^2( e^{-\frac{{\rm Mcdm}}{T}}-\frac{Y}{\tilde{Y}_{eq}})
        +    v\sigma_4Y^2( e^{-\frac{2{\rm Mcdm}}{T}}-\frac{Y^2}{\tilde{Y}_{eq}^2})
\end{eqnarray}
where $\tilde{Y}_{eq}=Y_{eq}e^{\frac{{\rm Mcdm}}{T}}$.  

\noindent
$\bullet$ \verb|darkOmegaExtTR(&Xf,TR, YR,vs_a, vs_s, vs_3,vs_4,&err)|\\
This routine is similar to darkOmegaTR. It solves Eq.~\ref{equExt} in the interval T$\in$[TR,Tend]  with the  condition Y=YR. Note that the function  {\tt darkOmegaExt}
described  above finds a starting point and calls {\tt darkOmegaExtTR}. The evolution of  the abundance can be accessed via function {\tt YF(T)}.     

At first, {\tt darkOmegaExtTR} builds the interpolation for  the external functions as required for a fast solution of the evolution equation. The result of the interpolation 
can be verified by functions {\tt vs\_aExt(T), vs\_sExt(T), vs\_3Ext(T), vs\_4Ext(T)}. 


\subsubsection{vSigmaCC}
...
\ref{cross_section}. The returned value  is given in [c$\cdot$pb].  Note that \verb|vSigmaCC| takes 
into account only the first subprocess in the {\tt cc} code, while the {\tt newProcess} routines allows generation of code for multi processes.

To prepare the code for the {\tt darkOmegaExt} routine, one needs $mode\ne0$, say, $mode=1$. If $mode\ne 0$, \verb|vSigmaCC|  calculates  the contribution of a given
...
\tilde\beta$, an additional factor $2$ if the incoming state is not self-conjugated.
 For processes with 3 and 4 outgoing particles {\tt vSigmaCC} checks  the number of odd outgoing particles  and adds 
factors $e^{\frac{M_{cdm}}{T}} $ and  $e^{\frac{2M_{cdm}}{T}} $  when needed. This means that the cross sections prepared by 
{\tt vSigmaCC} can be used directly in {\tt darkOmegaExt}.   
...
For this one has to first compute the corresponding $2\to 3$ and/or $2\to 4$ processes, this can be done with the functions  presented below 
in the narrow width approximation:\\

\noindent
$\bullet$ \verb|vSigmaPlus24(proc22, T, &err)|\\
calculates the contribution to $v\sigma$ for a $2\to 4$ process  associated with the  $2\to2$ process  for which outgoing particles are off-shell. 
...
%and  provide the user a possibility to split a sector with broken chemical
...
\subsection{Scenario   with late decaying field}
\label{sec:slowInflaton}
In the early Universe,  a field $\phi$ \footnote{We will assimilate the field $\phi$ with the inflaton, however our treatment is general and apply to any late decaying field.}
which decays into SM radiation will inject entropy in the SM bath and modify the evolution of the DM density. 
%During cosmic reheating, the inflaton $\phi$ is assumed to decay into SM radiation with a total
%decay width $\Gamma$.
 The dynamics of the background is driven by the set of Boltzmann equations
...
\label{Infl_evol_s}
   \frac{ds}{dt}&=&\Gamma\frac{\rho_\phi}{T} -3Hs
    \end{eqnarray}
 where  $\Gamma$ is the width of the late decaying field $\phi$ and $s=\frac{2\pi^2}{45}h_{eff}(T)T^3$.
 The Hubble expansion rate is given by 
 \begin{equation}
   H=\sqrt{\frac{8\pi}{3} \frac{ \rho_R+\rho_\phi }{M_P^2}} = \sqrt{H_{SM}^2+H_I^2}
   \label{eq:H}
   \end{equation}
...
solves Eqs.~\eqref{Infl_evol_rho} and~\eqref{Infl_evol_s}.  The solution is stored in the tabulated functions {\tt Ta(a)} and {\tt Ha(a)}, which respectively return 
the temperature and the Hubble parameter as a function of the scale parameter $a\in [1, \text{aEnd}]$.  Moreover the functions 
{\tt rhoIa(a)} and {\tt rhoSMa(a)} return the energy density of  the inflaton and SM bath respectively.
 These functions are available to the user. Here, \verb|HI| is the Hubble rate at the end of inflation when HI is generated by the inflaton only. \verb|Gamma| is $\Gamma_\phi$, the decay width of the inflaton. \verb|Trh, Tmax| and \verb|aEnd| are return parameters. They represent respectively the temperature when the energy density of inflation becomes equal to the energy density of SM particles, the maximal temperature reached, and the expansion factor at the temperature \verb|Tend|. This temperature is defined by the user as a global parameter.
 The function \verb|getInflDecay| returns 0 when it can find a solution to the differential equations or 128 otherwise.

   The function \\
$\bullet$\verb|aT(T)|\\
 returns the scale parameter $a$ at a given temperature $T<$~{\tt Tmax},  {\tt aT(T)} ={\rm NaN} when $T$ exceeds {\tt Tmax}. 
In general for  $T<$~{\tt Tmax}  there are two values of  $a$ corresponding  to a given temperature. {\tt aT} 
returns the largest of them  which corresponds to the cooling time  of the Universe.            


The function \\
$\bullet$\verb|getInflDecayPlus(Hsm, HI, Gamma, alpha, omega ,  &Trh, &Tmax, &aEnd)|\\
is and extended version of {\tt getInflDecay} above. Here {\tt Hsm} is the  SM bath contribution to the Hubble rate, Eq.~\ref{eq:H},
the parameters {\tt alpha} and {\tt omega}  ($\alpha$ and $\omega$) modify the evolution of the scalar field, 
\begin{equation}
 \label{eq:rhophi}
 \frac{d\rho_\phi}{dt}= -\Gamma(a) \rho_\phi -3(1+\omega)H\rho_\phi
\end{equation}
and  $\Gamma$ now depends on the scale parameter {\it a} 
\begin{equation}
  \label{eq:alphaw} 
  \Gamma(a) = \Gamma  a^{-\frac{ 8\alpha - 3(1+\omega)}{2}}
\end{equation}  


The evolution of the Dark Matter density $n$  is given   by the equation 
\begin{equation}
   \frac{dn_\chi}{dt}+3H n_\chi =-<v\sigma>(n_\chi^2-\bar{n}^2_\chi) + b \rho_\phi)
\end{equation}
which can be  conveniently rewritten in terms of $z_\chi=n_\chi a^3$
\begin{equation} 
\label{z_evolution}
  a\frac{d z_\chi}{d a}=-\frac{1}{H a^3}(<v\sigma>(z^2_\chi -\bar{z}^2_\chi) - b \rho_\phi a^6)
\end{equation}
where 
\begin{equation}
\label{eq:b}
b=\frac{1}{m_\phi} \sum_k k Br(\phi\to k \chi)
\end{equation}
and we assume  $ b m_\phi \ll 1$.


To solve for the relic density, we have two functions:\\
\noindent $\bullet$ \verb|darkOmegaInfl(b,Beps, &Tfo, &err) |\\
solves the DM evolution Eq.~\eqref{z_evolution} where {\tt b} is the branching fraction for the decay of the  inflaton into $N$ DM particles divided by  the inflaton mass in GeV, see Eq.~\eqref{eq:b}. This function  uses Ta, rhoIa, and Ha obtained by \verb|getInflDecay| or \verb|getInflDecayPlus|. The main return value of this routine is the DM relic density $\Omega_\chi h^2$. The auxiliary output parameters are {\it double } {\tt Tfo} and {\it integer} {\tt err}. The parameter {\tt Tfo}  returns the freeze-out temperature. Recall that in freeze-out the final relic density decreases when the annihilation cross section  increases, otherwise Tfo=NaN.\footnote{The C-function {\it isfinite} checks the number and returns 0 in the case of {\tt NaN}.} The error code {\tt err} indicates whether there is a problem with the solution of the differential Eqs.\eqref{Infl_evol_s} 
and~\eqref{Infl_evol_rho}, in which case the 8$^\text{th}$ bit of the error code {\tt err} is 1, or with the solution of Eq.~\eqref{z_evolution}, then the 9$^\text{th}$ bit is 1. The lowest bits contain information about problems with numerical integration and should be treated as warnings. The parameter \verb|Beps| has the same meaning as in other \verb|darkOmega| routines; it determines the condition to include Boltzmann suppressed channels~\cite{Belanger:2001fz}. The DM abundance is stored in an array and is available through the functions {\tt Za(a)}, \verb|Ya(a)| while the equilibrium abundance is stored in {\tt ZaEq(a)}, \verb|YaEq(a)|.\\

\noindent
$\bullet$ \verb|darkOmegaNInfl(b, Beps,  &err) |\\
solves the DM evolution 
 for $N$-component DM~\cite{Alguero:2023zol}. This function has the same argument and output as above except for \verb|Tfo|, this parameter is not included because some components can behave as WIMPs while others are FIMPs .
%except that here, for simplicity, we assume that the inflaton does not decay into DM. 
The DM abundance is stored via the functions {\tt ZaN(a,ch)}, \verb|YaN(a,ch)|, while the equilibrium abundance is stored in {\tt ZaNEq(a,ch)}, \verb|YaNEq(a,ch)| where \verb|ch| is a text which specifies the channel, for example {\tt ch="2"}. {\tt b} represents an array of coefficients that describe the branching fraction of the inflaton decay into DM components, see Eq.~\eqref{eq:b}. In particular, the first coefficient (that is, b[0]) corresponds  to the branching fraction of $\phi$ into the first DM component CDM[1].
 
The user has the possibility to define the parameters that enter \verb|getInflDecayPlus| 
 in the input  file. It can be done using the following records with numerical values, see example in IDM/infl2.par. The  input parameter  of the \verb|darkOmegaInfl| function can be defined in the same manner. 
\begin{verbatim}
#! HsmInfl ... 
#! HInfl ... 
#! GammaInfl ...
...
\end{verbatim}
where
where {\tt HsmInfl} and {HInfl} correspond to $H_{SM}$ and $H_I$ in Eq.~\ref{eq:H}, {\tt GammaInfl} is $\Gamma$ in Eq.~\ref{Infl_evol_s} , \verb|wInfl,alphaInfl| are $w,\alpha$ in 
Eq.~\ref{eq:alphaw} and \verb|BrInfl| is b in Eq.~\ref{eq:b}.
...
$q$ specifies the quark, $d,u$  or $s$. Heavy quark coefficients are calculated automatically. The form factors can be changed individually or recalculated all together with 
...
$F$ does not contribute to other processes. 
 ...
 In general we assume that DM-nucleon interactions do not depend on the recoil energy. This assumption is not valid when  DM-quark elastic scattering proceeds through a  light mediator. In this case, one cannot make a  direct comparison of  Eq.\ref{eq:norm}  with the  limits  on DM-nucleon cross section provided by experiments.  micrOMEGAs treats differently  two cases with a  light mediator,  the photon mediator which can induce milli-charge, dipole or quadruple interactions and  other mediators. For the photon mediator, the infinite contribution  to amplitudes  is subtracted  and  {\tt nucleonAmplitudes} returns an error code 4. If there is another  mediatior lighter than 100MeV which contributes to {\it SI} or {\it SD}  amplitudes, the error code is 1 or 2. The contribution of the light mediators to DM scattering on nucleus will however be taken into account as described next. 
...
mediator. {Note that the recoil energy distribution in a model with a light mediator  is different from the one used in analysis of  direct detection experiments, therefore it is necessary to recast  the results of the DD experiment to determine whether a  model is excluded or not, see Section  \ref{Recasting}.    }

For a complex WIMP and if DM has only one
...
The SI 90\%  DD  limits  tabulated from the results presented by LZ5T~\cite{Aalbers_2025}, XENON1T~\cite{Aprile:2018dbl}, DarkSide-50~\cite{Agnes:2018ves}, PICO-60~\cite{Amole:2019fdf}  and CRESST-III~\cite{Abdelhameed:2019hmk}  are accessible through  the following functions \\

\noindent
\begin{tabular}{|l| l|l|}
\hline
[{\color{blue}SI}]  name  &   region[GeV] & cite\\ 
\hline
 \verb|XENON_NT|       &   $ 3 < M_{DM}<12 $ & \cite{XENON:2024hup}    \\
 \verb|PandaXS2|       &   $ 2 < M_{DM}<6 $  & \cite{PandaX:2025rrz}    \\
 \verb|LZ5T|           &   $ 9 < M_{DM}$     &  \cite{Aalbers_2025} \\
 \verb|PandaX4T|       &   $ 5 < M_{DM}$     & \cite{PandaX-4T:2021bab} \\
 \verb|XENON1T|        &   $ 6 <  M_{DM}$   &\cite{Aprile:2018dbl}  \\
 \verb|DS50   |        &   $ 0.7 < M_{DM} < 15$    &\cite{Agnes:2018ves}   \\
 \verb|PICO60 |        &   $ 3 < M_{DM} $   & \cite{Amole:2019fdf}   \\
 \verb|CRESST_III |    &   $ 0.35 < M_{DM} < 12$   & \cite{Abdelhameed:2019hmk} \\
\hline 
\end{tabular}
%\noindent
...
\begin{tabular}{|l| l|l|}
\hline
[{\color{blue}SD}]  name  &   region[GeV] & cite\\ 
\hline
 \verb|PICO60_SDp|    &  $ 3 < M_{DM} $  & \cite{Amole:2019fdf} \\
 \verb|LZ5T_SDp|      &  $9 < M_{DM} $   & \cite{Aalbers_2025} \\
 \verb|LZ5T_SDn|      &   $9 < M_{DM} $  & \cite{Aalbers_2025} \\
 \verb|XENON1T_SDp|   &   $ 6 <  M_{DM} $ & \cite{Aprile:2019dbj} \\
 \verb|XENON1T_SDn|   &   $ 6 <  M_{DM}$ & \cite{Aprile:2019dbj} \\
 \verb|CRESST_III_SDn|& $ 0.35 < M_{DM} < 12$   & \cite{Abdelhameed:2019hmk}\\
\hline
\end{tabular}


These functions give the  excluded cross sections in ${\rm cm}^2$ units . For a DM mass outside the range specified the function returns
NaN. For experiments that present  an  exclusion limit for masses larger than the masses of nuclei in  the detector, we use the   extrapolation 
\begin{equation}
\nonumber
...
$\bullet$\verb|Darwin_XLZD(M)| which presents the projections for  DM exclusion with  the future experiment {\tt Darwin} \cite{Baudis:2024jnk} and \\
$\bullet$\verb|Neutrino_FloorXeSI(M)| which gives the neutrino floor limit for Xenon-like experimrnts \cite{OHare:2021utq}.  
 

\subsubsection{Recasting the experimental limits with  micrOMEGAs}  
\label{Recasting}

For the cases of light mediators, multi-component DM or  non-standard  DM velocity distribution,  the limits on the DM-nucleon cross-section obtained  by direct 
detection experiments cannot be applied directly. It is therefore necessary to perform a recasting of each DD  experiment and  recalculate  the experimental limit  
corresponding to a non-standard  recoil energy spectruml. The recasting for  {\tt XENON1T, DS50, PICO60 ,and CRESST\_III} was done in \cite{Belanger:2020gnr}, in this version we have added
 the  recasting of LZ5T experiment. Fig. \ref{fig:LZ5T}(left) demonstrates the  quality of our recasting for 
SI  interaction. Note that our exclusion limit is sligthly weaker in the range 10-30 GeV. 

\begin{figure}[htb]
\centering
\includegraphics[width=7cm]{LZ5Trecast.pdf}
\includegraphics[width=7cm]{DDmD.pdf}
\vspace{-.3cm}
\caption{ Left: recasting of LZ5T experiment. The orange curve is obtained by the  {\tt DD\_factorCS} routine.
Right: Exclusion of magnetic monopole by direct detection experiments.}
\label{fig:LZ5T}
...
[pb] units. The return value 0.1 corresponds to a 90\% exclusion.   
 The {\tt expCode} parameter can be any of the codes  \verb|LZ5T,XENON1T_2018,DarkSide_2018|, \verb|CRESST_2019,PICO_2019| or their combination concatenated with the symbol
$\mid$. There is also a predefined parameter that  currently combines these experiments 
\begin{verbatim}
  AllDDexp=LZ5T_2024|XENON1T_2018|DarkSide_2018|PICO_2019|CRESST_2019;
\end{verbatim}
...
\subsubsection{ Direct detection limit for photon exchange processes.}
As mentioned above, the contribution of a  photon mediator  is subtracted from  the $A^{SI}$ amplitude. The DM-nucleus amplitude is obtained  as the sum 
$A^{SI}$ and the electromagnetic amplitude. The latter is obtained using the   $DM,DM^*,\gamma$ vertex found in the file for the  Lagrangian  and the Coulomb $N,N^*,\gamma$
vertex for the  nuclei $N$. In Fig.\ref{fig:LZ5T} we show the limit on the  DM magnetic dipole obtained from a recasting of different experiments. The DM model with electric 
milli-charge, magnetic and electric dipole is presented in the {\tt dipoleDM} model included in micrOMEGAs.    
...
The {\tt Emax} parameter is assigned to {\tt SpectArray[0]}.\\
...
The {\tt spin2} parameter is 0 for bosons and 1 for fermions. The parameter {\tt med=0} if the particle and antiparticle are produced via a
scalar mediator and $1$ for a  vector mediator. {\tt E} is the energy of the  photon.    Here we use formulas 4.8 - 4.11  
of Ref.~\cite{Coogan_2020}. Note that {\tt FSRdNdE} contains the photon spectrum  generated by  both particle and anti-particle.   
...
\nu_{\mu},\nu_{\tau},D^-\} $$
The {\tt Mass} parameter defines the mass of the DM particle. {\tt basicSpectra} returns an error code if the  parameter {\tt outN} (err=1) or {\tt pdg} ( err=2) is out or range.   
The spectrum  depends on  


%{\tt SpectraFlag}, {\color{blue} {\tt SpectraFlag=0} corresponds to micrOMEGAs own spectra, 
%but {\tt SpectraFlag>0} specifies the spectra obtained by  external packages, see below. For external packages parameter 
%the {\tt Mass} parameter has to be larger than 5~GeV.
%}

% Note that  the antideuterons spectra is provided  only within  PPPC.  
% which specifies the external package used to calculated the  spectra, see below.
...
$\bullet$   \verb|SpectraFlag| \\
 is a switch that allows to choose the tables that contain the spectra. The default value {\tt 0} corresponds to micrOMEGAs' spectra, while other values {\tt 1,2,3}
activate spectra generated by external codes. Note that the spectra for the {\tt Mass} parameter smaller than 5GeV are available only in
 micrOMEGAs' tables while  the antideuterons spectra are  provided  only within  PPPC {\tt SpectraFlag=2}.     
...
{\tt SpectraFlag=0} corresponds to  new tables for $\gamma, e^+,\bar{p},\nu$ recently obtained for micrOMEGAs using Pythia8
  for  DM in the range  $0.5{\rm  GeV} - 10{\rm TeV}$. \footnote {These tables replace the original {\tt micrOMEGAs} tables obtained with Pyhia6, these can be accessed with {\tt SpectraFlag=-1}}.
...
$23+'L'$  provides the spectra produced by a polarized Z boson.  We  have also included the spectra corresponding to  DM annihilation into $\pi^0$, $\pi^{\pm}$, $K^0$, $K^\pm$, and  $\eta$ mesons. The lower limit is defined by the particle mass.\\
...
which has the same parameters as {\tt }. 
...
The test code  {\tt mdlIndep/basicSpectra.c} contains an example of a call to {\tt basicSpectra} and compares the four different spectra that are included in micrOMEGAs,   checks energy conservation and shows uncertainties.  \\
  ...
  The functions presented below use the spectra for elementary processes obtained by {\tt basicSpectra} and model dependent matrix elements.
...
determines whether the photon spectra {\tt SpA}  is compatible with the 95\% limits obtained from 
observations of Dwarf Spheroidal galaxies  Carina, Fornax, LeoI, SegueI,  Draco, LeoII,
 Sculptor, Sextans
 \cite{Albert_2017, Bonnivard_2015, Alvarez_2020}. Here {\tt vSigma} is the  DM annihilation cross section at low velocities( $<v\sigma>$  in [$cm^3/s$] units)  as obtained by \verb|calcSpectrum|.  
 The return value more or equal to 1  indicates  that the given model is excluded at 95\% confidence level. 
 The tested $<v\sigma>$ divided  on the return value should be close to 95\% excluded cross section. But this relation is approximate. In particular because
 return value is always  in unterval $0.01 - 100$.
...
choose  the   neutrino spectra. The default value  {\tt WIMPSIM=0} \footnote{Since \pppc\ does not provide neutrino spectra produced at the center of the
...
\noindent $\bullet$ \verb|atmNuFluxes(nuPdg,E,cos)| \\
returns the atmospheric  neutrinos fluxes in $( m^2\cdot sec\cdot sr\cdot GeV)^{-1}$ units  as presented in \cite{Honda:2015fha}. 
Only $\nu_e,\bar\nu_e,\nu_\mu,\bar\nu_\mu$ are included corresponding to PDG codes,  nuPdg =  12,-12,14,-14. Input parameters should be specified in the range,
$-1\le cos\le 1$,  $ 0.1 \le E \le 10^4$GeV.\\
Ref.~\cite{Honda:2015fha} provides  several tables of calculated neutrino fluxes depending on disposition, 
solar activity, with and without  mountains over the detector. By default we use the  file\\
 {\it "grn-ally-20-01-mtn-solmax.d"} which gives the neutrino flux for a detector 
under  the mountains in Gran Sasso at the time of large solar activity. To use other tables provided  in\\
 {\tt http://www-rccn.icrr.u-tokyo.ac.jp/mhonda/public/nflx2014/index.html} \\
the user has to copy the corresponding file in the micrOMEGAs directory {\tt Data/nu\_data} or in the  current directory and  write down the name of the file in  the public variable {\it atmNuFile}. For example\\
\verb|   atmNuFile="grn-ally-20-01-mtn-solmin.d";|\\


\noindent $\bullet$ \verb|atmNuFluxesI(nuPdg,E)| \\    
...
\section{ Relic density of sterile neutrinos.}
$\bullet$ \verb|darkOmegaNu(T0,Tinit, kL, sin22, LsInit, msKeV)| \\
calculates the relic density of sterile neutrinos. Here we assume that the sterile neutrino mixes with the SM  
left handed neutrino  of generation {\tt kL}. The parameter {\tt sin22}$=sin^2(\phi/2)$ defines the mixing angle. 
The parameter {\tt LsInit} is the lepton asymmetry, L,  in the {\tt kL}'th  generation   at temperature {\tt Tinit} normalized to  
the entropy density.  
Note  that SM interaction does not change L/s. {\tt msKeV} is the mass of the sterile neutrino in 
{\tt keV}.  
The temperature {\tt T0} is usually  chosen to be equal to {\tt 0.01GeV} and {\tt Tinit} has to be 
of the oder of  several GeV's 
since the  generation  of the relic density of sterile neutrinos  occurs at  a temperature  around 100-200 {\tt MeV}.    

Note that lepton asymmetry is changed by the  resonance production of sterile neutrino. In general 
\begin{equation} 
  Y=Y_{\nu_s}+Y_{\bar{\nu}_s} \approx Y_{\nu_s} - Y_{\bar{\nu}_s} = \frac{L(T_{init})}{s(T_{init})} - \frac{L(T_0)}{s(T_0)}  
\end{equation}
Thus the rate of DM generation follows the  rate of disappearance of lepton asymmetry. The latter is represented 
by the function \\ 
$\bullet$ \verb|LasymmS(T)|\\
which is given by $\frac{L(T)}{s(T)}$.  \\
The resulting momentum distribution   in terms of 
$\epsilon=p/T$  for $T << 1MeV$  is presented by the function \\
\noindent
$\bullet$ \verb|fNuSterile(|$\epsilon$\verb|, txt)|\\   
where the text parameter {\tt txt}  allows one  to specify the distribution of sterile neutrino, {\tt +nus}, antineutrino,  {\tt -nus} or 
total one, {\tt nus}.   With the parameter  {\tt txt} one can define an additional factor of power 
of $\epsilon$. 
 For example, the number density of neutrino at temperature T is 
\begin{equation}
   \rho= \frac{4\pi}{(2\pi)^3} T^3 \int\limits_0^\infty fNuSterile(\epsilon,"nus2") d\epsilon  
\end{equation} 
 


Our calculation follows that of \cite{Venumadhav_2016} and we have roughly a 15\%   numerical  agreement  with this paper. In particular,  the  momentum distribution  {\tt fNuSterile} can be compared with Fig. 10(b,d)  and {\tt LasymmS} can be compared with Fig. 10a.  See 
the test code  in {\tt mdlIndep/nuS.c}.  
...
The functions {\tt LiLithMO} and {\tt hbBlocksMO}    need information about numerical values of   couplings included in Higgs interactions with SM particles.  We use the tools presented in Section \ref{sec:Vertices}  to  find them. 
...
\micro\ is now interfaced to version 3.1.0 of \smodels, which, among other novelties, features an improved treatment of width-dependent constraints. 
...
\begin{quote}
  \verb|smodels(Pcm, nf, csMinFb,ExcludeBSM, fileName, version, wrt)|
\end{quote}  
...
(Note that $u$, $d$, $\bar{u}$, $\bar{d}$  and gluons are always included  while  $s$, $c$, and $b$ quarks are included for ${\tt nf} = 3,4,5$ respectively.). 
 The new version of \smodels\   also considers  $2 \to 1$  reactions  
\begin{quote}
       \verb|proton,proton -> BSM|  
\end{quote} 
All $2 \to 2$  and $2 \to 1$ reactions involving BSM particles are computed by micrOMEGAs, the user can however simplify this task by providing the list of particles that do not need to be considered in the  parameter {\tt ExcludeBSM}. In this list, particle names must be separated  by commas or spaces.  The corresponding antiparticles are taken into account  automatically. 
...
The call of \verb|smodels()| is per default done in the file \\ \verb|micromegas_X/include/SMODELS.inc| 
...
\begin{quote}
    \verb|micromegas_X/include/smodels_parameters.ini|.
 \end{quote}
...
Note that, for \smodels, PDG code 25 is reserved for a SM-like Higgs and should not be assigned generically.  Thus,  
micrOMEGAs re-assigns the  PDG codes  when needed. Namely  the particle which  satisfies  the conditions above  is assigned the  code 25, while a  particle with  code $25$ that does not satisfy one of these conditions is re-assigned the  code
$25252525$.   
 

%\tcb{\subsection{Precision $Z$ Observables, LEP Neutral-State Searches, and LHC Limits on Heavy Neutral Gauge Bosons}}
...
\noi$\bullet$ \verb|ZpLimCMS(ZpName)|\\
returns value more than 1 if its interaction with quarks and leptons is excluded  at 95\%  confidence level (C.L.) according to the CMS analysis of dilepton resonance production based on 140~fb$^{-1}$ of Run~2 data~\cite{CMS:2021ctt}.  
The argument \texttt{ZpName} is the name of the $Z'$ particle in the Beyond--Standard--Model (BSM) extension under investigation, as defined in the corresponding \texttt{CalcHEP} model file.

\begin{equation}
L=  Z'_\mu \bar{q} \gamma^\mu(g_v+g_a\gamma^5) q
\end{equation}
\begin{equation}
  c_q=(g_v^2 + g_a^2) (Br(Z'\to e^+,e^-) + Br(Z'\to \mu^+,\mu^-))
\end{equation}
  The couplings and branching rations are  obtained using tools presented in Sec. \ref{sec:Vertices} and \ref{sec:Decays}. 
 The [$c_d$,$c_u$] defined in CMS experiment  are disposed on a straight line. We find the boundary points of this line 
[cdMax($M_{Z'}$),0] and [0,cuMax($M_{Z'}$)] \footnote{ See section  
{\tt \#ifdef find\_cu\_cd\_max} in  ZpPortal/main.c}. Then {\tt ZpLimCMS}
returns   $c_d/cdMax(M_{Z'})+c_u/cuMax(M_{Z'}) $
...
parameters of the model as  specified in SLHA. The spectrum calculator is \verb|NMSPEC|~\cite{Ellwanger:2006rn}
 in the \verb|NMSSMTools_6.1.0| package~\cite{nmssmtools}.
...
The files {\verb|*.c|} in the directory {\tt mdlIndep}  contain examples of the calculation of the direct detection,  indirect detection 
and neutrino telescope signals using the routines described in this section. 
The numerical input data  corresponds to 'MSSM/mssmh.dat'.  The file {\tt mdlIndep/mySigmaV.c}   contains an example of the computation of the relic density for a constant cross-section, for this one needs to define 

\noindent
$\bullet$ \verb|constSigmaVpbc|\\
which is the address of a variable which contains $<v\sigma>$[pb] that will be used in the routines {\tt darkOmega, darkOmegaTR, darkOmegaInfl}.
By default this parameter is {\tt NULL}.   

\section{Squared Matrix Elements, cross sections, decay widths.}
\label{cross_section}
...
\subsection{Squared matrix elements with CalCHEP.}
...
\noindent
$\bullet$ \verb|numout*cc=newProcess(procName)|\\
...
automatically.  The library  is loaded automatically and  the {\tt newProcess } routine returns the  address of the loaded code ({\tt cc})\footnote{ The {\tt struct numout} is described in 
{\tt CalcHEP\_src/c\_source/dynamicME/include/dynamic\_cs.h}  
and 
{\tt CalcHEP\_src/include/num\_out.h}
} . The  second call to {\tt newProcess} with the same {\tt procName}  does not lead to  recompilation of the library as long as   the model is  not changed.  Moreover the library and its name depend on the {\tt ForceUG} flag.
  A multichannel process can be generated using  {\it N}{\tt*x}, for example, {\tt e,E->2*x}  
% corresponding to the process.
% If the library {\tt libName} already exists, it is not recompiled.
If the process can not be compiled, then a NULL address is
returned.  
...
$\bullet$ \verb|cc=getMEcode(twidth, Gauge, procName,excludeVirtual,excludeOut, libName)|\\
compiles the  codes for any $2\ra 2$ or  $1\ra 2$  reaction while allowing
 to exclude some  set of  virtual  and outgoing  particles listed in {\tt excludeVirtual} and {\tt excludeOut} respectively. 
 The {\tt tWidth} parameter allows (0) or forbids (1)  t-channel widths in  the generated matrix elements. The {\tt Gauge} parameter specifies unitary or Feynman gauge.
 The library name has to be defined by the user.  Note, that this name is included in the names of different procedures and thus  cannot contain $\pm$ and other symbols  not allowed for names of C-routines.
 The {\tt procName} argument simulates  input process in CalcHEP GUI mode. 
If {\tt procName} contains words which do not correspond to particle names, these words are treated as names of composite particles and GUI session of CalcHEP requires their decoding, see Fig.\ref{fig:scr}
To support this facility  {\tt procName} uses the '\{' symbol to separate input lines which in GUI mode is realized by  pressing  the {\tt Enter} key. To organize the input process corresponding to Fig.\ref{fig:scr} we need \\
\verb| procname=Proton,Proton -> LL,LL- {u,U,d,D{e,E,m,M |\\ 



\begin{figure}[htb]
\centering
\includegraphics[width=8cm]{scr.pdf}
\vspace{-.3cm}
\caption{ 
}
\label{fig:scr}
...
\noindent
$\bullet$\verb|passParameters(cc)|\\
passes independent and constrained parameters to the code {\tt cc } and forces it to calculate internal constraints. If calculation of some internal constrained parameters leads to {\tt NAN}, then   non-zero error code is returned. The integration functions presented in sections \ref{sec:SQMEint}, \ref{sec:sqme_MC} call the {\tt passParameters} routine automatically.


\subsection{ Testing  the  content of the code for matrix elements.}
...
\noindent
$\bullet$\verb| address->interface->pinf(nsub, np, &mass, &pdg)|\\
returns directly the  name of $np^{th}$  particle in subprocess $nsub$. It also   returns auxilliary information about the particle: its  mass\footnote{ Parameter {\tt mass}  must be  be of  type of {\tt REAL} which is defined in {\tt CalcHEP\_src/interface/nType.h} } and  PDG code.  

%
\subsection{ Integration of matrix elements.} 
\label{sec:SQMEint} 
The squared matrix element  corresponding  to the compiled code {\tt cc} can be  calculated using  the function\\
$\bullet$\verb|cc->interface->sqme(nSub,GG,pvect,NULL,&err)|\\
where
\begin{itemize}
\item {\tt GG} is the value of the  strong coupling
\item {\tt pvect}
is an array of incoming and outgoing momenta. 
Elements of {\tt pvect} have type {\tt REAL}.
\item  The energy  of $n^{th}$ particle  is stored in  {\tt pvect[4*(n-1]}$>0$. So, the conservation law reads $\sum_{n\in in} p_n=\sum_{n\in out} p_n $ 
\item Momenta have to be presented in {\tt GeV} units. Then  return value is presented in  $GeV^{8-2(nin+nout)}$. 
 \end{itemize}

\noindent
$\bullet$ \verb|cs22(cc,nsub,P,c1,c2,&err)|\\
...
\subsection{CalcHEP Monte Carlo  session}
\label{sec:sqme_MC}
Tthe user  has the possibility to run  a CalcHEP Monte Carlo session replacing  CalcHEP CUI interface by  direct calls to C-routines. Before starting a MC session  the user can specify the cuts for phase space and histograms.\\
$\bullet$\verb| addCut(key, Parameter, min, max)|\\
restricts the region  of integration by the condition\\
\verb|      min < Parameter < max ,              key>=0   | \\
\verb|   Parameter  < min  or  max < Parameter,  key<0| \\ 
The nomenclature for  {\tt Parameter}  can be found in ~ \cite{Belyaev:2012qa} Sect. 4.3. For instance,\\  
\verb|   addCut(1, "M(m,M)",500,600)|,\\
 applies a cut on the invariant mass of $\mu^+$, $\mu^-$ pairs.
The  task  to fill a histogram  during a MC integration  is set  by the  instruction \\
$\bullet$ \verb|addHistogram(Parameter, min, max)|\\

The MC session is initiated by the command \\
$\bullet$ \verb|vegasGrid * vegPrt=initMCsession(cc, nSubProc, P1,P2,STF1,STF2)|\\
where {\tt nSubProc} is the number of the subprocess  in the code {\tt cc};
{\tt P1,P2} are momenta of incoming particles; {\tt STR1}, {\tt STR2} are structure functions in  CalcHEP notation(see above).  For example: \\
{\small \verb|vegPtr=initMCsession(cc,1,6.8E3,6.8E3, "PDT:cteq66(proton)","PDT:cteq66(proton)") |}\\  
...
$\bullet$ \verb| vegas_int(vegPtr, Ncall, alpha, Nproc, &res, &dres)|\\
where {\tt Ncall} is the number of calls  of the squared matrix element, {\tt nProc} is the  number of processors used for parallel calculation, {\tt res} and {\tt dres } are  respectively the   cross section obtained by the  MC integration  and its uncertainty in [pb] units. The  scale $Q=\sqrt{s}$ is used both for the QCD coupling   and for structure  functions. After each session {\tt vegas} improves the integration grid. In general the  first calls of \verb|vegas_int|  have a  large  integration uncertainty  which decreases  step by step in further calls. 
The parameter {\tt alpha } defines the rate of grid improvement.  In the case {\tt alpha=0} the integration grid is stable. The recommended  value is  {\tt alpha=1.5}.   We recommend to exclude the  contributions of the few  first {\tt vegas}  sessions  from histogram statistics. It can be done by the command\\
$\bullet$\verb|clearHist()|\\

In the end of MC calculations one can save histograms in the file \\
$\bullet$\verb|saveHistograms(fileName)|, \\
remove cuts and histograms and clean the  integration grid by the commands  \\
$\bullet$\verb|delCuts()|\\
$\bullet$\verb|delHistograms()|\\
$\bullet$\verb|vegas_finish(vegPtr)|.\\
The code \\
\verb|     CalcHEP_src/bin/show_distr   fileName |\\
allows to visualize the histograms generated.\\

It is  to sum several distributions obtained in different sessions using\\ 
\verb|     CalcHEP_src/bin/sum_distr fileName1  fileName2 .. > fileSum|\\
One can find an example of a Monte Carlo session in {\tt mdlIndep/MCsession.c}.
...
gives  an address where information about the branchings is stored, see below the description of routines which use this parameter. 
By default {\tt pWidth} works in the following manner (note that this can be modified  for each particle by the {\tt pWidthPref} command described below):

For models which read a SLHA parameter file, the values of the widths and branchings are taken from the SLHA
file when \verb|useSLHAwidth=1| .\\[2mm] If \verb|useSLHAwidth=0| or the decay of a  given particle is not presented in the SLHA file, then \micro\ applies its own 
calculation of width.  
If the  \verb|1->2| decay channels are kinematically accessible 
then by default  only these channels are
...
\noindent
$\bullet$\verb|pWidthPref(pName, pref)| changes the mode of the {\tt pWidth} calculation individually for the particle {\tt pName}. Here {\tt pref=4} restores the default {\tt pWidth} mode. Otherwise the 
...
%
\subsection{ Extraction of couplings for vertices.}
\label{sec:Vertices}
  ...
  %The first one collects the symbolic information about the vertex\\
$\bullet$ \verb| lVert* VVV =getVertex(pN1,pN2,nP3,pN4);|\\    
extracts the symbolic expression in the four- vertex where {\tt pN\it{i}} are the names of particles in the vertex. For three-leg vertices set {\tt pN4=NULL}. The command returns the address of the structure 
which contains  the following information \\
\begin{verbatim}
    VVV->nTerms        - the number of terms 
    VVV->SymbVert[k]   - the symbolic formula for the  terms, 0<=k<VVV->nTerms
\end{verbatim} 

For example, to get information on the {\it E~e~Z}   vertex which in  {\tt lgrng1.mdl} is written as
\verb!   E    |e    |Z    |     |EE/(4*CW*SW)      |4*SW^2*G(m3)-G(m3)*(1-G5) !\\
we use the command \\
\verb|    lVert* VVV=getVertex("E","e","Z",NULL); |\\
which will return
\begin{verbatim}
    VVV->GGpower=0 
    VVV->nTerms=2
...
\end{verbatim}
Note that the particle ordering in {\tt getVertex} does not have to match the one in the Lagrangian and  the Lorentz index corresponds to the position of the vector particles in the call of {\tt getVertex}, not to their position in the Lagrangian table.\\
 $\bullet$ \verb|getNumCoeff(VVV,coeff);| \\  
provides the 
numerical coefficients  for all {\tt SymbVert},
here the numerical  array {\tt coeff} should contain at least {\tt VVV->nTerms} elements. Note that {\tt GGpower} -  the  power of strong coupling included in the vertex -  does not contribute to the numerical coefficient.



\section{Implementation of  a new model}
...
his/her own  |\verb|lib/Makefile| if need be.
...
programs, main.c and main.cpp. In these sample main programs it is assumed that the input parameters are provided in a separate
...
\subsection{Tools  for model files generation}
...
Such tools are 
 \texttt{LanHEP}~\cite{Semenov:2014rea}, \texttt{FeynRules}~\cite{Alloul_2014}, and \texttt{SARAH}~\cite{Staub_2013} .  
 LanHEP is included in micrOMEGAs and can be found in {\tt Packages/LanHEP}.  It can be compiled by calling {\tt make} which generates the executable file {\tt lhep}.    Generation of  model files in CalcHEP/micrOMEGAs notations is launched  by the command \\
   \verb|       lhep -ca source_file|\\
  Most of the models included  in micrOMEGAs are accompanied with the LanHEP  sources that can be found in the  {\tt work/lanhep}  directory.
  The user can find instructions for writing  LanHEP source files in the directory \verb|Packages/LanHEP/manuals|\\ 
...
See details in Section \ref{sec:Decays}.
...
$N^4LO$ ( 5 loops) formula in \cite{ParticleDataGroup:2020ssz, Baikov_2017}. For $\alpha_{n_df-1}(\alpha_{n_f})$
at  falvour thresholds  $m_c(m_c),\; m_b(m_b),\; m_t(pole)$  we use formulas \cite{Chetyrkin_2006}.

\noindent$\bullet$ \verb| MtRun(Q), MbRun(Q), McRun(Q) | \\
calculates top, bottom and charm quarks running masses evaluated
at $N^4LO$ \cite{Baikov_2014}.
...
curves/histograms  to display,  It should not exceed 15.
...
The function\\
$\bullet$\verb|darkOmegaInfl(|$H_I$,$\Gamma$, Beps,\&$a_{end}$,\&Trh,\&isWIMP, \&err)\\
calls {\tt getInflDecay} and calculates relic density in the thermal scenario caused by slow inflaton decay.
It  solves the evolution equation for the DM number density 
\begin{equation} 
 \label{Infl_evol_dm} 
...
The evolution of  the DM abundance is  stored in an  array and is  available  via the functions {\tt Za(a)}, {\tt ZaEq(a)}, \verb|Ya(a)|, {\tt  YaEq(a)}  and are defined in interval $[1,a_{end}]$. 
% \footnote{ The function {\tt YaEq(a)} can be constructed as {\tt Yeq(Ta(a)}}  defined in the interval $ a \in [1,a_{end}]$. 