...
int smodels(int Run, int nf,double csMinFb, char * ExcludeBSM, char*fileName, char*version, int wrt) 
{ 
   int SMP[nSMP]={1,2,3,4,5,6, 11,12,13,14,15,16, 21,22,23,24,25, 111, 211};
   int i,j;
   int nChan=0;
...
//  QNUMBERS
   ...
   {  
      ...
      int isBSMparticle=1;
      for(j=0;j<nSMP;j++) {
         if(abs(newPDG(ModelPrtcls[i].NPDG))==SMP[j]) {
           isBSMparticle=0;
           continue;
         }  
      } 
   ...
   { 
     for(j=0;j<nSMP;j++) if(abs(newPDG(ModelPrtcls[i].NPDG))==SMP[j]) break; 
     if(j==nSMP)
     {
        np++; 
   ...
   for(i=0;i<nModelParticles;i++) 
   {  for(j=0;j<nSMP;j++) if(abs(newPDG(ModelPrtcls[i].NPDG))==SMP[j]) break;
      if(j==nSMP) newSlhaDecayPrint(ModelPrtcls[i].name,0,f); 
   }

   char*ExcludeBSM_=NULL;
   if(ExcludeBSM) 
   {   ExcludeBSM_=malloc(strlen(ExcludeBSM)+3);  
       ExcludeBSM_[0]=' ';
       strcpy(ExcludeBSM_+1,ExcludeBSM);
       int l=strlen(ExcludeBSM_);
       ExcludeBSM_[l]=' '; 
       ExcludeBSM_[l+1]=0;
       for(int i=0;i<l;i++) if(ExcludeBSM_[i]==',') ExcludeBSM_[i]=' ';  
   }     

   printf("computing LHC cross sections 2->2... \n");
   ...
   for(i=0;i<np;i++) for(j=i;j<np;j++) if(pMass(plist[i])+pMass(plist[j])<PcmMax)
    ...
    {  
       ...
       {  
          char nm[40];
          sprintf(nm," %s ", plist[i]); 
          if(strstr(ExcludeBSM_,nm)) continue;
          sprintf(nm," %s ",antiParticle(plist[i]));
          if(strstr(ExcludeBSM_,nm)) continue;
          sprintf(nm," %s ", plist[j]);
          if(strstr(ExcludeBSM_,nm)) continue;
          sprintf(nm," %s ",antiParticle(plist[j]));
          if(strstr(ExcludeBSM_,nm)) continue;
       }
...
/*pb*/         fprintf(f," 0  0  0  0  0  0 %E # CalcHEP %s\n\n", dcs,VERSION);
  ...
  printf("computing LHC cross sections 2->1... \n");


   for(i=0;i<np;i++)  if(pMass(plist[i])<2*PcmMax) if(plist[i][0]!='~') 
    {  
       if(ExcludeBSM_)
       {  
          char nm[40];
       ...
       qNumbers(plist[i], NULL, &q31,&c1);
       q32=0,c2=1; //      qNumbers(plist[j], NULL, &q32,&c2);
       ...
       {  double dcs;
          ...
          {    
             ...
             if(dcs>csMinFb*0.001)
             {
               fprintf(f,"XSECTION  %E   2212  2212  1  %d \n",2*Pcm, newPDG(pNum(plist[i]))); 
/*pb*/         fprintf(f," 0  0  0  0  0  0 %E # CalcHEP %s\n\n", dcs,VERSION);
               nChan++;
             }
             if(Pcm==PcmMax) break; else Pcm=PcmMax;
  ...
  fclose(f);
  free(plist);
  if(ExcludeBSM_) free(ExcludeBSM_);
...
int  smodels_(int*LHCrun, int *nf,double *csMinFb, char*ExcludeBSM, char*fileName, char*version, int *wrt, int ExcludeLen,  int len1,int len2)
{
  char cName1[30];
  fName2c(fileName,cName1, len1);
  char cName2[30];
  fName2c(version,cName2, len2);
  char cExcludeBSM[300];
  fName2c(ExcludeBSM,cExcludeBSM, ExcludeLen);
     
  return smodels(*LHCrun,*nf, *csMinFb, cExcludeBSM, cName1,cName2,*wrt);
}
