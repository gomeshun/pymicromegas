...
#define vC  299792.                   //  speed of light  in   [km/s]  


int DDLflag=1;
#define QCDcorrections   // A switch On QCD corrections for  heavy quark  contribution 
#define  Twist2On        // A switch to include contribution of twist-2 operators

extern double DDmomSQ;     

static int numDM=0;                                // position of DM particle in particle table.
static numout*ccSI=NULL,*ccSD=NULL,*ccN=NULL;      // Codes of DM-SI, DM-SD, DM-nucleus  elastic scattering.
                                                   // If numDM=0 the  codes are compiled and are recompiled if numDM is changed 
 
#define INMAX 100                                  // The size of table of particles involved in DM-nucleon interections.
                                                   // Actually it is the DM and all color particles 


static int getAuxCodesForDD(char*pname)
{ 
  // Try find to load previously  generated libraries for DD
  char libnameSI[40]="DDauxSI";
  char libnameSD[40]="DDauxSD";
  char libnameNN[40]="DDauxNN";

  pname2lib(pname,libnameSI+strlen(libnameSI));
  pname2lib(pname,libnameSD+strlen(libnameSD));
  pname2lib(pname,libnameNN+strlen(libnameNN));

  ccSI=getMEcode(0,1,"",NULL,NULL,libnameSI);
  ccSD=getMEcode(0,1,"",NULL,NULL,libnameSD);
  ccN =getMEcode(0,1,"",NULL,NULL,libnameNN);
  
  if(ccN)  return 0;  // in general ccSI and ccSD can be NULL. not ccN 
 
//   Creates a list of particles involved in the DD calculation. They are DM and  color particles
   ...
   for(int i=0;i<INMAX;i++) pInvolved[i].num=0;   
   
   for(line=0,K=1;line<nModelParticles;line++)
   { 
     char * name1, *name2; 
     int num; 
     int color;
     ...
     if((strcmp(pname,name1)==0||strcmp(pname,name2)==0||(abs(color)!=1 && abs(num)!=21)))
     {   int k=0; 
         if(color!=1) k=K++;
         pInvolved[k].num=num;
   ...
   for(int i=1;i<K;i++) if(pInvolved[i].num>=-6 && pInvolved[i].num<=6 && strcmp(pInvolved[i].mass,"0")==0)
...
//  We create extended model with  DM-quarks point like interaction needed to extract DD couplings and NULCEUS
  int newDir=0;
  int err=0;
  
    newDir=prepareWorkPlace(); // creates  working directory with a copy of the model    
    char * command=malloc(strlen(WORK)+strlen(calchepDir)+7000); // it should be enough 

//  vars1.mdl add:  new parameters
   sprintf(command,"cd %s;  %s/bin/s_calchep -blind \"{[[{{\\19\\${" //goto the last line of vars1.mdl \\19 = ^Y, \\09=\t
      "{M_NUCLEUS\\09130\\09"                            //  mass of nucleus    
      "{SC_NUCLEUS\\090\\09"                             //  nucleus scalar coupling
      "{VC_NUCLEUS\\090\\09"                             //  nucleus SD vector coupling
      "{EE_NUCLEUS\\090\\09"                             //  nucleus electromagnetic coupling        
   "}}0\"",compDir,calchepDir);
   err=system( command);   
   if(err) {free(command); printf("err1\n"); exit(0); return 1;}


//  func1.mdl: new constrained parameters   
   sprintf(command,"cd %s;  %s/bin/s_calchep -blind \"[[{[{\\19\\${" //goto the last line of func1.mdl \\19 = ^Y , \\09=\t  
    "{M_aux\\091"                      //  M_aux=1  for 'mass' of point-like mediator 
    "{sqrt6D\\092.449489742783178"     //  new  parameter sqrt6D=sqrt(6) constant     
   "}}0\"",compDir,calchepDir);
   err=system( command);

// prtcls1.mdl: moves  DM to  the last position in the model particle list  
   sprintf(command,"cd %s;  %s/bin/s_calchep -blind \"[[{[[{" 
   "\\19%d{\\04\\19\\${{}}0\";",compDir,calchepDir,pInvolved[0].line);       // \04=^D delete
   err=system( command);
   if(err) {free(command);return 2;}

// prtcls1.mdl: new particles, nucleus and auxilary ones   
   sprintf(command,"cd %s;  %s/bin/s_calchep -blind \"[[{[[{"   
   "{_S0_\\09_S0_\\09_S0_\\090\\090\\09M_aux\\090\\091\\09*\\09_S0_\\09_S0_"   // new point-like scalar _S0_
   "{_V5_\\09_V5_\\09_V5_\\090\\092\\09M_aux\\090\\091\\09*\\09_V5_\\09_V5_"   // new point-like vector _V5_
   "{NUCLEUS\\09NUCLEUS\\09aNUCLEUS\\091001300540\\091\\09M_NUCLEUS\\090\\091\\0954\\09NUCLEUS^+\\09NUCLEUS^-"   //  NUCLEUS
   "}}0\";",compDir,calchepDir);
   err=system( command);
   if(err) {free(command);return 3;}

   char toFact[10],aP_P[30];  
   for(int i=0;i<Valence-2;i++) toFact[i]='\t'; toFact[Valence-2]=0;  // to skip empty field positions in the Lagrangian table

// lgrgn1.mdl:  interaction terms  of auxillary fields with  color particles    
   sprintf(command,"cd %s; %s/bin/s_calchep -blind \"[[{[[[{",compDir,calchepDir);
   for(int i=1;i<K;i++) 
   { sprintf(aP_P,"%s\\09%s\\09",pInvolved[i].aname,pInvolved[i].name);
     if(pInvolved[i].spin2==0)
     {
       sprintf(command+strlen(command),
       "{%s_S0_%sM_aux\\091",aP_P,toFact);          // (s S _S0_) M_aux   interaction for scalars  
     }else
     if(pInvolved[i].spin2==1)
     {  sprintf(command+strlen(command),     
       "{%s_S0_%sM_aux\\091"                  //   (F f _S0_)   M_aux interaction for fermions     
       "{%s_V5_%sM_aux\\09-G5*G(m3)"          //   (F f _V5_)  -M_aux*G5*G(m3) interaction
       ,aP_P,toFact,aP_P,toFact);        
     } 
   }
...
// lgrgn1.mdl:  interaction terms  of auxillary fields with  DM
   sprintf(command,"cd %s; %s/bin/s_calchep -blind \"[[{[[[{",compDir,calchepDir);

   sprintf(aP_P,"%s\\09%s\\09", pInvolved[0].aname, pInvolved[0].name);
   switch(pInvolved[0].spin2)
   { 
    case 0: sprintf(command+strlen(command),
              "{%s_S0_%s4*M_aux*%s\\091",aP_P,toFact,pInvolved[0].mass); // (DM  dm _S0_)*4*M_aux*Mdm  interaction
            break;
    case 1: sprintf(command+strlen(command),                                                
              "{%s_S0_%s2*M_aux\\091"                             // (DM  dm _S0_)*2*M_aux 
              "{%s_V5_%s2*M_aux\\09G5*G(m3)"                      // (DM  dm _V5_)*2*M_aux*G5*G(m3)
            ,aP_P,toFact,aP_P,toFact);
            break; 
    case  2:sprintf(command+strlen(command),                                    // for vector DM:    
             "{%s_S0_%s2*M_aux*2*%s\\09m1.m2"                     //(DM dm _S0_)*2*M_aux*Mdm*m1.m2
             "{%s_V5_%s2*i*M_aux*sqrt6D\\09eps(m1,m2,(p1-p2),m3)" //(DM dm _V5_)*2*i*M_aux*sqrt(6)*eps(m1,m2,(p1-p2),m3)
             ,aP_P,toFact,pInvolved[0].mass,aP_P,toFact);
   }  
   strcat(command, "}}0\"");
   err=system(command);
   if(err) {free(command); return 5;}   

// lgrgn1.mdl:  interaction terms  of auxillary fields with  NUCLEUS
   sprintf(command,"cd %s; %s/bin/s_calchep -blind \"[[{[[[{",compDir,calchepDir);
   sprintf(command+strlen(command), 
         "{NUCLEUS\\09aNUCLEUS\\09_S0_%sM_aux*SC_NUCLEUS\\091"
         "{NUCLEUS\\09aNUCLEUS\\09_V5_%sM_aux*VC_NUCLEUS\\09 -G5*G(m3)",toFact,toFact
         );
   strcat(command, "}}0\"");
   err=system(command);
   if(err) { free(command); return 6;}

   char*photon=pdg2name(22);    
   if(photon)  // lgrgn1.mdl: interaction of NUCLEUS with photon 
   { 
      sprintf(command,"cd %s; %s/bin/s_calchep -blind \"[[{[[[{",compDir,calchepDir);
      sprintf(command+strlen(command),
        "{NUCLEUS\\09aNUCLEUS\\09%s%sEE_NUCLEUS\\09G(m3)",photon,toFact);
      strcat(command, "}}0\"");
      err=system(command);
      if(err) { free(command); return 7;}
   }    
   free(command);  
//  The end of edditing of the  model        

   char ProcNameSI[4000], ProcNameSD[4000], ProcNameDmN[100];;
   char exclude[30];
   ...
   sprintf(ProcNameDmN,"NUCLEUS,%s -> NUCLEUS,%s",pname,pname);    

  if(ProcNameSI[0])
  {  sprintf(exclude,"_S0_!=1,_V5_");
     if(photon ) sprintf(exclude+strlen(exclude),",%s",photon); 
  ...
  if(ProcNameDmN[0]) ccN=getMEcode(0,1,ProcNameDmN,NULL,NULL,libnameNN);
  
  if(ccSI) *(ccSI->interface->BWrange)=0;
  if(ccSD) *(ccSD->interface->BWrange)=0;
     
  if(newDir) cleanWorkPlace();  // delite auxillary working directory. One can save it to check auxillary model. 
  return 0;
}
...
{
  pA0[0]=0; pA0[1]=0;  nA0[0]=0; nA0[1]=0; pA5[0]=0; pA5[1]=0;  nA5[0]=0; nA5[1]=0;
  
  int pos=pTabPos(WIMP);  
  if(!pos) return -1;
  
  if(pos!=numDM)  { if(getAuxCodesForDD(WIMP)) { printf("can not calculate DD amplitudes\n"); return -1;} else numDM=pos;} 
  if(!ccSI && !ccSD) return 0;    
  pos--; 
   
  int Qnum,aQnum,i,II,sgn,ntot,n;
  double  s;
  numout *cc; 
  ...
  double wimpMass=pMass(WIMP),MN=0.939;
...
#ifdef Twist2On                        
        if(aQnum<6) /* twist-2  SI amplitude  */   
        { int k;
          double g=0,gp,gn;
          double h=masses[0]/100;
          double d4[3]={0.5,-1,0.5};
...
/*  OnlyTEQ0=0; */
  ...
  int err=0;
  numout*allcc[3]={ccSI,ccSD,ccN};
  for(int i=0;i<3;i++)if(allcc[i])
  { int ntot;
    procInfo1(allcc[i],&ntot,NULL,NULL);
    for(int nsub=1;nsub<=ntot;nsub++) for(int n=1;;n++)
    {  int m,w,p;
       char*s=allcc[i]->interface->den_info(nsub,n,&m,&w,&p);
       if(!s) break;
       if(s[0]==1 && s[1]==3)  // t-channel propagator
       {   if(i==2 || allcc[i]->interface->va[m]<0.1) err=err|(1<<i); }
    }
  }    
  return err; 
}
...
static double vfv(double v){ double v0=1; if(v<v0)  return v*_fv_(v0)/v0/v0; else return _fv_(v)/v;}
...
static  double vfvA(double v,void*e )
{ 
  double E=*(double*)e;
  REAL m[4]; 
  int pdg[4];
  for(int k=0;k<4;k++) ccN->interface->pinf(1,k+1,m+k,pdg+k);
  double Mr=m[0]*m[1]/(m[0]+m[1]); 
  double p=Mr*v/vC, pp=p*p;
  double z=m[0]*E/pp;
  REAL co=1-z,si= sqrt(z*(2-z));
  if(co<=-1) { co=-1,si=0;}               
  REAL P[16]={0};
  for(int k=0;k<4;k++) P[4*k]=sqrt(m[k]*m[k] + pp);
        
  P[3]=p;P[7]=-p;
  P[8+3]=p*co;
  P[8+2]=p*Sqrt((1-co)*(1+co));  
  if(pdg[0]!=pdg[2]) 
  { P[8+3]*=-1;
    P[8+2]*=-1;
  }    
  ...
  double sqrtS=P[0]+P[4];
  int err=0;  
  double cs=ccN->interface->sqme(1, 0.12,P,NULL, &err)/(32*M_PI*sqrtS*sqrtS);
  return cs*vfv(v); 
}

int  nucleusRecoil1(char *WIMP, double(*fv)(double),
      int A, int Z, double J,
      void (*Sxx)(double,double*,double*,double*),
      double * dNdE)
{
  ...
  DDmomSQ=0; 
  int na=nucleonAmplitudes(WIMP,pA0,pA5,nA0,nA5); 
  if(na<0 || !ccN) return 1;     // we can not caclulate WIMP-nucleon amplitudes
  REAL*M_NUCLEUS=NULL,*EE_NUCLEUS=NULL,*SC_NUCLEUS=NULL;  

  for(int i=1;i<=ccN->interface->nvar; i++) 
  { 
//     printf("i=%d name=%s\n",i, ccN->interface->varName[i]);
     if(ccN->link[i]) ccN->interface->va[i]=*(ccN->link[i]);
     else if(strcmp(ccN->interface->varName[i],"M_NUCLEUS")==0)  M_NUCLEUS=ccN->interface->va+i;    
     else if(strcmp(ccN->interface->varName[i],"EE_NUCLEUS")==0) EE_NUCLEUS=ccN->interface->va+i;
     else if(strcmp(ccN->interface->varName[i],"SC_NUCLEUS")==0) SC_NUCLEUS=ccN->interface->va+i;
  }
   
//  if(!M_NUCLEUS||!EE_NUCLEUS||!SC_NUCLEUS||!VC_NUCLEUS) return 2;   // WIMP-nucleon amplitudes are not calculated properly. It can not be.
  double MA=0.94*A, Mdm=pMass(WIMP), Mr=MA*Mdm/(MA+Mdm);
  *M_NUCLEUS=MA;
  if(EE_NUCLEUS) *EE_NUCLEUS=Z*sqrt(4*M_PI/137.);
  double  x0=Mdm/(Mdm+MA), E0=2*MA*x0*x0;
  _fv_=fv;
  double  vmax=vEsc+vEarth; 
   
  int nAnti =  strcmp(WIMP,antiParticle(WIMP))? 2:1;  //  1 in case of selfconjugated DM, and 2 otherwise   
  for( int k=0;k<nAnti;k++)
  {
     if(SC_NUCLEUS)*SC_NUCLEUS=Z*pA0[k]+(A-Z)*nA0[k];  
//*SC_NUCLEUS=0;   // to test SD part
     if(k&& EE_NUCLEUS ) *EE_NUCLEUS*=-1;        
     ccN->interface->calcFunc();   
     double C= nAnti==1? 1 :  (1+dmAsymm*(1-2*k))/2;    // asymmetry factor ? 
     for(int i=0;i<RE_DIM;i++) 
     {
       double E=RE_START*1E-6*pow(RE_STEP,i);
       if(na)  // small t-channel masses
       {
         DDmomSQ=2*MA*E;
         nucleonAmplitudes(WIMP,pA0,pA5,nA0,nA5);
         if(SC_NUCLEUS)*SC_NUCLEUS=Z*pA0[k]+(A-Z)*nA0[k]; 
         ccN->interface->calcFunc(); 
       } 
             
       double vmin=sqrt(E/E0)*vC;
       if(vmin>=vmax)  { dNdE[i]=0 ;continue;}
       double p=sqrt(E*MA*2)/0.197327;         //  transverce momentum in 1/fm ? 

       double dndeSI= MA/Mr/Mr*simpson_arg(vfvA,&E,vmin,vmax,1.E-4,NULL);  // main term 
       dndeSI*=pow(FermiFF(A,p),2);            // nucleuSCcoeffs form factor 
       double dndeSD=0;
       if((pA0[k]!=0 || nA5[k]!=0) && J && Sxx)  
       { double s00,s01,s11;                     //  form factors
         Sxx(p,&s00,&s01,&s11);                   //  calculation of form factors 
         double AVplus =pA5[k]+nA5[k];
         double AVminus=pA5[k]-nA5[k];                     
         dndeSD= 8*MA*(AVplus*AVplus*s00+AVminus*AVminus*s11+AVplus*AVminus*s01)/(2*J+1); // squared matrix element with form factors
         dndeSD*=simpson(vfv,vmin,vmax,1.E-4,NULL);  // velocity integral
       }  
       dNdE[i]+=(dndeSI+dndeSD)*C;                          //  
     }   
  }
  DDmomSQ=0;
  for(int i=0;i<RE_DIM;i++) dNdE[i]*=0.1*vC*vC*(rhoDM/Mdm)*Kg/MA*lDay*3.8937966E8*1E-36;
  return 0;
}
