...
static double T_;
static double M1,M2,sqrtSmin,sqrtSmax,massSing,Mgap;
...
{  double sv_tot,t,bess, x1,x2,y;
   ...
   improveCrossSection(pdg[0],pdg[1],pdg[2],pdg[3],PcmIn,T_,&sv_tot);
...
{
   double z=1-x[0]*x[0];
   double sqrtS=sqrtSmin-3*T_*log(z);    // sqrt(s)
   double J=6*T_*x[0]/z;                 // Jacobian  d sqrt(s)/dx[0]

   double M34_min=pmass[i3]+pmass[i4],M34_max,M34;
   
   if(massSing) 
   { 
     if(sqrtS>=sqrtSmax) return 0;    
     M34_max=sqrtSmax-pmass[i5];
     M34=M34_min + x[1]*(M34_max-M34_min);
     if(M34+pmass[i5]>sqrtS) return 0;   
   } else 
   { M34_max=sqrtS-pmass[i5];
     M34=M34_min + x[1]*(M34_max-M34_min);
   } 

   REAL pvect[20];
   double pcmIn=decayPcm(sqrtS,pmass[0],pmass[1]);
   double phV=kinematic_23(pcmIn,i5, M34, 2*x[2]-1 ,2*x[3]-1,2*M_PI*x[4],pmass,pvect)*2*4*M_PI*(M34_max-M34_min);
   double GG=sqrt(4*M_PI*alphaQCD(sqrtS));
   double x1=M1/T_, x2=M2/T_, y=sqrtS/T_,u=Mgap/T_;
   double bess=    sqrt(2*x1*x2/y/M_PI)*exp(-(y-u-x1-x2))*K1pol(1/y)/(K2pol(1/x1)*K2pol(1/x2));
   
   int err;
   return    pow(2*M_PI,4)/(4*pcmIn*sqrtS)     //  kinematic factor at ME^2
           * phV                               //  phase volume            
           * CI->sqme(1,GG, pvect,NULL,&err)   //  squared matrix element 
           * pow(sqrtS*pcmIn/M1/M2,2)/T_*bess  //  weght for ME^2*dsqrtS  integration
           * J                                 //  d sqrtS/dx[0]
           ;     
}
...
static double vsigma23integrand0(double *x, double w)
{
   ...
   r*=4*pow(M_PI,4)/(pvect[0]+pvect[4]);  // because kinematic_23 was changed.
...
static double vsigma24integrandT(double *x, double w)
{
   ...
   sqrtS=sqrtSmin-T_*log(x[0]);
   J=T_/x[0];
   
   double M34_min=pmass[i3]+pmass[i4], M56_min=pmass[i5]+pmass[i6], M34_max, M56_max, M34, M56;

   if(massSing) 
   { if(sqrtS>sqrtSmax) return 0;
     M34_max=sqrtSmax-M56_min;
     M34=M34_min + x[1]*(M34_max-M34_min);    
     M56_max=sqrtSmax-M34_min;
     M56=M56_min + x[2]*(M56_max-M56_min);
     if(M34+M56>sqrtS) return 0;   
   } else    
   { 
     M34_max=sqrtS-M56_min;
     M34=M34_min + x[1]*(M34_max-M34_min);
     M56_max=sqrtS-M34;
     M56=M56_min + x[2]*(M56_max-M56_min);
   } 

   REAL pvect[24]; // momenta
   double pcmIn=decayPcm(sqrtS,pmass[0],pmass[1]);   
   double phV=kinematic_24(pcmIn,i3,i4, M34, M56,  2*x[3]-1 ,2*x[4]-1,2*x[5]-1, 2*M_PI*x[6], 2*M_PI*x[7],pmass,pvect)
                  *(M34_max-M34_min)*(M56_max-M56_min)*2*4*M_PI*4*M_PI; // phase space volume 
         
   double GG=sqrt(4*M_PI*alphaQCD(sqrtS)); 
   int err;
   double bess=    sqrt(2*M1*M2/(sqrtS*T_*M_PI))*exp(-(sqrtS-Mgap -M1-M2)/T_)*K1pol(T_/sqrtS)/(K2pol(T_/M1)*K2pol(T_/M2));
          //    bessK1(sqrtS/T_)/bessK2(M1/T_)/bessK2(M2/T_)*exp(2*Mcdm/T_);
//printf("M1=%E M2=%E Mcdm=%E\n",M1,M2,Mcdm);              

//printf("sqre=%E\n", CI->sqme(1,GG, pvect,NULL,&err));
...
static double chFraction(double T, char*p0,char*p1)    // return a factor which transforms vSigma for separate process(mode=0) 
{                                                      //  to contribution to vSigma for all DM annihilation (mode=1) 
  double M1=pMass(p0),M2=pMass(p1);
  char *c0=NULL,*c1=NULL;
  double s=0,k=pow(M1*M2,1.5)*K2pol(T/M1)*K2pol(T/M2);            
  ...
  {  double m=pMass(ModelPrtcls[i].name);
     int dim=ModelPrtcls[i].cdim*(ModelPrtcls[i].spin2+1);
         ...
         s+=dim*pow(m,1.5)*K2pol(T/m)*exp(-(m-0.5*(M1+M2))/T); 
      }
      if(k)      
      {  if(strcmp(p0,p1)) k*=2;
...
double vSigmaCC(double T,numout* cc, int mode)
{
  ...
  double Cab=1; if(strcmp(pname[0],pname[1])==0) Cab=0.5;
  ...
  { if(pname[0][0]!='~' || pname[1][0]!='~') return 0;
    ...
    int nDmOut=0;
    for(int i=0;i<CI->nout;i++) if(pname[2+i][0]=='~') nDmOut++;
    if(nDmOut>2) Mgap=(nDmOut-2)*Mcdm; 
  }   
 
  
  sqrtSmin=M1+M2;
  double sm=0; for(int i=0;i<CI->nout;i++) sm+=pmass[2+i];
  if(sqrtSmin<sm)
  { if(T==0) return 0; else sqrtSmin=sm; }
  sqrtSmax=sqrtSmin-T*log(bEps); 

  massSing=0;
  i3=2;i4=3;i5=4;i6=5;
  if(CI->nout>2) for(n=1;(s=CI->den_info(1,n,&m,&w,NULL));n++)
  if(strlen(s)==2 && s[0]>2 && s[1]>2) 
  {  
     double mm=0,ww=0;
     if(m) mm=fabs(CI->va[m]); if(w) ww=CI->va[w];
     if(mm>pmass[s[0]-1]+pmass[s[1]-1] && mm< sqrtSmax) 
     { double dS=exp(-(mm-sqrtSmin)/T)/mm/ww; 
       if(dS > massSing) {i3=s[0]-1; i4=s[1]-1; massSing=dS;}    
     }
  }
  ...
  { 
     case 2:
       ...
       {  double eps=1.E-3;
          sqme22=CI->sqme;
          nsub22=1;
          a=simpson(u_integrand_,0.,1.,eps,NULL);
       }   
       break;
     case 3:
     {          
        if(T==0) a=vegas_chain(3, vsigma23integrand0 ,2000,1.5, 0.01,&dI,NULL);
        else     a=vegas_chain(5, vsigma23integrandT ,2000,1.5, 0.01,&dI,NULL);
        break;
     }
     case 4:

     if(T==0) a=vegas_chain(6, vsigma24integrand0 ,4000,1., 0.03,&dI,NULL);
     else     a=vegas_chain(8, vsigma24integrandT ,4000,1.2, 0.005,&dI,NULL);
...
//   WIDTH_FOR_OMEGA=0;
   ...
   if(mode) a*= chFraction(T,pname[0],pname[1]);

   if(Qaddress && CI->nout==2) { *Qaddress=oldQ; calcMainFunc();}
 
   return 3.8937966E8*a;   // pb*c units
}
