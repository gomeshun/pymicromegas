...
//printf("1-y=%E Tx=%E sqrtS=%E\n", 1-y,Tx,sqrtS);  
  ...
  if(kin22_par(arg,sqrtS, sqrt(4*M_PI*parton_alpha(sqrtS)))){  printf("!kin22 problem sqrtS=%e %E \n",sqrtS, arg->sqrtSmin); return 0;}
  err=0;
  arg->T=T;
  sqme_Int=sqmeIntDel(arg,eps/3);

  double cs=sqme_Int/(32*M_PI*sqrtS)*arg->PcmOut/arg->PcmIn;
  improveCrossSection(arg->pdg[0],arg->pdg[1],arg->pdg[2],arg->pdg[3],arg->PcmIn,T,&cs);
  
  double res=  cs*T/(8*M_PI*M_PI*M_PI*M_PI)*arg->PcmIn*arg->PcmIn
...
//printf("Masses=%e %e %e %e\n", (double)arg->pmass[0], (double)arg->pmass[1],(double)arg->pmass[2],(double)arg->pmass[3]);  
  ...
  intervals[0]=0;  intervals[1]=1-sqrt(1-exp(-sqrtSmin/(3*T)));  
                   intervals[1]=exp(-sqrtSmin/(3*T))/(1+sqrt(1-exp(-sqrtSmin/(3*T))));
...
//printf("nIntervals=%d\n", nIntervals);  
  ...
  { int err;
  char * nm[4];
  for(int i=0;i<4;i++) nm[i]=arg->cc->interface->pinf(arg->nsub,i+1,NULL,NULL);
//printf("process %s %s -> %s %s  T=%E\n", nm[0],nm[1],nm[2],nm[3],T);
//printf("sqrtSMin=%E, masses= %e %e %e %e\n", arg->sqrtSmin, pMass(nm[0]), pMass(nm[1]), pMass(nm[2]), pMass(nm[3]));
...
// printf("Proc=%s\n",Proc);      
   ...
   if(!err)
   {   
//displayPlot("lnTIntegrand", "log(T)",ltGrid[0],ltGrid[tGridDim-1], 0,1,"integrand", 0, lnTIntegrand,&arg); 
       double Y=0;
       for(int nT=tGridDim-2;nT>=0;nT--)  
       {  double lnT1= ltGrid[nT+1], lnT2=ltGrid[nT];
          if(nT==tGridDim-2) Y+=simpson_arg(lnTIntegrand,&arg,lnT2,lnT1,1E-3,NULL);  else    
                             Y+=gauss_arg(lnTIntegrand,&arg,lnT2,lnT1,4);
...
//double K1=bessK1(sqrtS/T);
   ...
   double cs= cs22(arg->cc,1,pcm, cos1, cos2, NULL)/3.8937966E8;
   int pdg[4]; 
   for(int i=0;i<4;i++) arg->cc->interface->pinf(1,i+1,NULL,pdg+i); 
   improveCrossSection(pdg[0],pdg[1],pdg[2],pdg[3],pcm,T,&cs);
   double  res= J*pcm*Pcm*sqrtS*sqrtS*K1*cs *Kout/(cos2-cos1)*2;
