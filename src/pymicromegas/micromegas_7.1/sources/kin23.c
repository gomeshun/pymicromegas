...
//  return 3.8937966E8*pcm1*pcm2/(512*M_PI*M_PI*M_PI*M_PI*(pvect[0]+pvect[4])*(pvect[0]+pvect[4]));
  return 2*M_PI*pcm1*pcm2/(8*(pvect[0]+pvect[4])*pow(2*M_PI,9)); 
}  
...
{ 
  ...
  {  double fi=M_PI*(0.5+i)/Np,ds;
     ...
     factor *=4*pow(M_PI,4)/(pvect[0]+pvect[4]);
...
double cs23(numout*cc, int nsub, double Pcm, int i3,int*err) { return 3.8937966E8*cs23Pcm(cc, nsub, Pcm, i3, err)/Pcm;}
...
double kinematic_24(double Pcm, int i3,int i4, double M1, double M2, double xcos,double xcos1, double xcos2, double fi1, double fi2,
                      REAL*pmass, REAL * P)
{ 
  REAL factor,pcm,p1cm,p2cm,chY,shY,xsin,sqrtS;
  REAL p0,p3;
  int i,j,i5,i6;  
...
for(i=0;i<4;i++)
{ REAL sum=P[0+i]+P[4+i]-P[8+i]-P[12+i]-P[16+i]-P[20+i];
...
//    if(Fabs(m-pmass[i])>pmass[i]*1.E-3) { printf("wrong mass %d (%E != %E) \n",i,(double)m,(double)pmass[i]); /*exit(33);*/}
  }
...
{ int err; 
  ...
  double  factor;
  factor=kinematic_23(Pcm_, i3, Mmin+x[0]*(Mmax-Mmin), c2_*2*(x[1]-0.5) , 2*(x[2]-0.5), M_PI*x[3],pmass,pvect);
  if(factor==0) return 0; 
  return  factor* (Mmax-Mmin)*(2*c2_)*2*(2*M_PI)*(cc23_->interface->sqme)(nsub_,GG, pvect,NULL,&err);
}
    ...
    double*dcs, double *chi2) 
{
  ...
  vegas_finish(vegPtr);
  double  sqrtS=Sqrt(Pcm*Pcm+pmass[0]*pmass[0])+Sqrt(Pcm*Pcm+pmass[1]*pmass[1]);  
  double C=3.8937966E8* pow(2*M_PI,4)/(4*Pcm*sqrtS);
  (*dcs)*=C;
  return rVal*C;
}
