...
int COSMO=0;  // 0 -FreezeOut, 1-FreezeIn, 2-Inflaton
double HInfl=1E-8;
double GammaInfl=1E-17;
double BrInfl=0;
double HsmInfl=0;
double alphaInfl=0.1875 ;
double wInfl=0;


double * constSigmaVpbc=NULL;  
...
{  int l,l_;
   ...
   int err;
   brV1=AUX[nsub22].br;
   r=cs23Pcm(cc23,1,PcmIn,AUX[nsub22].i3,&err)/brV1;
...
static  double sigma(double PcmIn,double T)
{ double r;
...
#ifdef IMPROVE
  improveCrossSection(pdg[0],pdg[1],pdg[2],pdg[3],PcmIn,T,&r);
#endif
...
{ double sum=0; int l;
  if(constSigmaVpbc) return K2pol(1/(Mcdm/T));
...
//nPROCSS=20;
  ...
  sprintf(pathtocalchep,"%s/CalcHEP_src",micrO);
...
static double aRate(double X, int average, int only,  double * alpha, aChannel ** wPrc, int *NPrc)
{
  ...
  if(constSigmaVpbc)
  {  if(alpha) *alpha=0;
     return (*constSigmaVpbc)/3.8937966E8;
  } 
...
double vSigmaMem(double T)
{ if(Mcdm/T < vSigmaGrid.xtop) return NAN;
  if(Mcdm/T > vSigmaGrid.xtop*pow(XSTEP,vSigmaGrid.pow)) return NAN;
  double alpha; 
  return  vSigmaI(T, 1E-5,&alpha)*3.8937966E8;
}  
...
double YF(double T)
{
     if(Ntab<=0 || T<Ttab[Ntab-1] || T> Ttab[0]) return NAN;
     if(T<Ttab[1] && T>Ttab[2])  return  exp(polint1(T,Ntab,Ttab, lYtab)); 
     return  exp(polint3(T,Ntab,Ttab, lYtab));
}


static double Hinv(double s3){ return s3*s3/Hubble(T_s3(s3));}
...
//    if(odeint(&y,1 ,Mcdm/Xt , Mcdm/X2 , 1.E-3, (Mcdm/Xt-Mcdm/X2 )/2, &XderivLn)){ printf("problem in solving diff. equation\n"); return -1;}
  ...
  double iColl;
  if(constSigmaVpbc)
  {   double s3t=s3_T(Mcdm/Xt), s3e=s3_T(Tend); 
      iColl=(*constSigmaVpbc)/3.8937966E8*simpson(Hinv,s3e,s3t,1E-3,NULL); 
  } else 
  {
    iColl=( (Mcdm/Xt)*sqrt(M_PI/45)*MPlanck*aRate(Xt,1,0,NULL,NULL,NULL));
...
{ _fast_=(Fast>0);
  ...
  if(Yr>0) lYtab[0]=log(Yr); else lYtab[0]=log(Yeq(Tr)*1E-30);
  Yt=Yr;
  int init=0;
...
//    alpha=vSigmaGrid.alpha[i];
    ...
    if(!init)
    { 
       double ddY= dY(s3_2,Beps);
       double Yeq_=Yeq(Ttab[i]);      
       if(ddY< 0.1) { Yt=Yeq_*sqrt(1+ddY);  lYtab[i]=log(Yt); } 
       else { init=1; }
    }
    if(init)
    { 
       if(odeint(&Yt,1 ,s3_t , s3_2 , 1.E-3, (s3_2-s3_t)/2, &XderivLn))
       { printf("problem in solving diff. equation T in [%.3E, %.3E] \n",Ttab[i-1],Ttab[i]);  if(err) *err=128|simpson_err;  return NAN;}
         lYtab[i]=log(Yt);
    }
  ...
  if(err) *err=simpson_err;
  if(!constSigmaVpbc){ for(int i=1;i<=Ncdm;i++) fracCDM[i]=0; fracCDM[wimp]=1;}
...
static double geff1_(double T)
{
  ...
  if(_beps_>0)    massCut-=T*log(_beps_); else  massCut+=1.E20;
...
//  Y[1]=-Yeq2(TR)+Y2R;
  ...
  if(wimp1==0)
  {
    ...
    for(int i=1;i<=Ncdm;i++) fracCDM[i]=0; fracCDM[wimp2]=1;
  ...
  } else if(wimp2==0)
  {
    ...
    for(int i=1;i<=Ncdm;i++) fracCDM[i]=0; fracCDM[wimp1]=1;
    if(err_) *err_=simpson_err;
...
double gCDM(int k, double T)                        //  =  2pi^2 n_eq(T) exp(Mcdm/T)/T 
...
static double pin2Sigma22(REAL Pcm, cs22IntSt* arg)  //  sqme intergated over cos excluding t-channel poles with factor pIn*pOut/(32*pi*s)= pIn^2*cs
{
  ...
  if(sqrtS<=mass[0]+mass[1] || sqrtS<=mass[2]+mass[3]  ) return 0;
  ...
  numout*cc=arg->cc;
   ...
   for(int n=1;;n++)
   {
     ...
     if(s[0]==1 && s[1]==3)
     {
        if(   (mass[0]< mass[2]+mt || mass[3]< mass[1]+mt )
           && (mass[1]< mass[3]+mt || mass[2]< mass[0]+mt ) ) continue;  // no problem for any energy          
         cosPole= -(t130- mt*mt )/(pp);
         if(cosPole>-lC && cosPole<lC) return 0;
     }else if (s[0]==1 && s[1]==4)
     {
         if(   (mass[0]< mass[3]+mt || mass[2]< mass[1]+mt )
            && (mass[1]< mass[2]+mt || mass[3]< mass[0]+mt ) ) continue;  // no problem for any energy      
...
//                dSum=simpson_arg(dSdC,arg, intervals[2*i], intervals[2*i+1],1E-2,&err);
                dSum=gauss345_arg(dSdC,arg, intervals[2*i], intervals[2*i+1],1E-3,&err);
  ...
  improveCrossSection(pdg[0],pdg[1],pdg[2],pdg[3],arg->Pin,arg->T,&sum);
  sum*=arg->Pin*arg->Pin;
...
static double u_integrand_vs( double u,void*argVoid )
{
   ...
   double J=6*T*(1-u_)/z;                   // Jdu=dSqrtS
   double cs=pin2Sigma22(Pcm,arg);          // cs*Pcm^2
   double bess=pow(sqrtS/T,1.5)*exp(-(sqrtS-arg->sqrtSmin)/T)*K1pol(T/sqrtS);   // K1(sqrtS/T)*sqrt(2/pi) * (sqrtS/T)^2
   return  cs*bess*J;
}
...
static double vsVFun( double PcmIn, void*arg_)
{
   ...
   double r=cs23Pcm(arg->cc23,1,PcmIn,arg->i3,&err)/arg->br;
...
static double vSigmaNstat(double T, vSigmaNArg* arg)
{
   ...
   for(cL=arg->cList; cL; cL=cL->next)    // cycle for compilation of processes which satisfies Beps condition
   {
     ...
     if( cL->cc==NULL  &&  cL->mMin  < arg->mMin  - TlogBeps )   
     { char process[50];
     ...
     if( cL->cc && cL->cc!=noll &&  cL->t < sortOddTime && !Qaddress) { passParameters(cL->cc); cL->t=sortOddTime; }
   }
...
//   printf("            %E %E -> %E %E\n", pMass(cL->pName[0]),pMass(cL->pName[1]),pMass(cL->pName[2]),pMass(cL->pName[3]));   
//      if(T<=cL->T0 || cL->cc == noll || cL->cc==NULL  )  { printf("T0 check:T=%E T0=%E  cc=%p noll=%p\n",T,cL->T0,cL->cc,noll); continue;}  // T0 check instead of Beps  one


      if( cL->mMin > arg->mMin -TlogBeps  ) continue; //else printf("T=%E cL->mMin =%E  arg->mMin =%E  -TlogBeps=%E\n",T, cL->mMin, arg->mMin, -TlogBeps); 

      if(Qaddress) { *Qaddress=cL->mMin+T; calcMainFunc(); passParameters(cL->cc); cL->t=sortOddTime;}
      ...
      for(int i=0;i<4;i++) if(cL->pdg[i]==21 || cL->pdg[i]==22 )
      {  int j;
         for(j=i+1;j<4;j++) if(cL->pdg[i]==cL->pdg[j]) break;  
         if(j==4)  //  only one A/G
         { double mS=0,mL=0;
           for(j=0;j<4;j++) if(j!=i) { double m=fabs(*cL->mAddress[j]); mS+=m; if(mL<m) mL=m;}
//            printf("mS=%E mL=%e\n",mS,mL);
           if(mL > mS-mL) isDecay=1;    // both processes A,Z->X,Y  and  Y,Z -> X,A in case Z->X,Y(bar)  are calcelled 
         }
         break;   
      }   

//      if(cL->pdg[i]==22) dm=T*0.31/sqrt(6); else {  dm=T*sqrt(4*M_PI*alphaQCD(T));} //thermal mass of photon or gluon

      if(isDecay) { /*printf("%s,%s ->%s,%s isDecay\n",cL->pName[0],cL->pName[1],cL->pName[2],cL->pName[3]);*/ continue;} // exclude A/G + decay 
      { REAL m0=*cL->mAddress[0], m1=*cL->mAddress[1];
       ...
       intervals[0]=minu; intervals[1]=1;
        ...
        for(int n=1;;n++)
        {
           ...
           { printf("m=%d mm=%E argI.sqrtSmin=%E     zero width in s-channel propagator %s in %s %s ->%s %s  %E %E -> %E %E \n",m,mm,
             (double)argI.sqrtSmin, ModelPrtcls[abs(pnum)].name,cL->pName[0],cL->pName[1],cL->pName[2],cL->pName[3], 
                   pMass(cL->pName[0]),pMass(cL->pName[1]),pMass(cL->pName[2]),pMass(cL->pName[3]) );
                   
            passParameters(cL->cc);
            
            printf("m=%E mm=%E argI.sqrtSmin=%E     zero width in s-channel propagator %s in %s %s ->%s %s  %E %E -> %E %E \n",fabs(cL->cc->interface->va[m]),mm,
             (double)argI.sqrtSmin, ModelPrtcls[abs(pnum)].name,cL->pName[0],cL->pName[1],cL->pName[2],cL->pName[3], 
                   pMass(cL->pName[0]),pMass(cL->pName[1]),pMass(cL->pName[2]),pMass(cL->pName[3]) );
        ...
        if(!isfinite(res)) 
        {   printf("T=%e   infinite energy integral for process %s %s-> %s %s \n",T, cL->pName[0], cL->pName[1],cL->pName[2],cL->pName[3]);
            cL->vcs=0;   
        }
        cL->vcs=res;
        vcsSum+=cL->vcs;
...
#define SMOOTH_T_DEP(T)  ( T *sqrt( T ) ) 

static double vSigmaNstatLogT(double logT, void* argVoid)
{ 
  vSigmaNArg* arg=(vSigmaNArg*) argVoid;
  double T=exp(logT);   double res= vSigmaNstat(T,arg)/SMOOTH_T_DEP(T);  return res; 
}
...
{
      double mMin=-1;
      int noVP=0;   // exclude virtual W/Z from co-scattering because of problem with t-channel poles
//      if((allChannels[k].types[0]==0 || allChannels[k].types[1]==0) && (allChannels[k].types[2]==0 || allChannels[k].types[3]==0)) noVP=1;

      if(Qaddress){ *Qaddress=2/*GeV*/;  calcMainFunc(); }
...
//      displayPlot("vSigmaNstatLogT","logT",log(Tend), log(Tstart),0,1,"int",0,vSigmaNstatLogT,allChannels+j);  
     }
  }
  return NULL;
}
...
if(T<Tend) T=Tend;    
    for(int n=0;n<nChannels;n++) if(!allChannels[n].Excluded)
    {
        ...
        double vcs,vcsR;
        if(tab) vcs= SMOOTH_T_DEP(T)*polint3(log(T),buildChan[n].dim,buildChan[n].lnT,buildChan[n].vs);
...
//        double res;
        if(interpol)
        {
          ...
          if(lnT<buildChan[n].lnT[0] || lnT>buildChan[n].lnT[dim-1]) return NAN;
           ...
           double res=SMOOTH_T_DEP(T)*polint3(lnT,dim,buildChan[n].lnT,buildChan[n].vs);
...
{  aChannel*outAr=NULL;
   ...
   if(n==nChannels) { *vsPb=0; return NULL;}
...
double Za(double a) 
{   
   if(Ntot_a==0 || Z_a==NULL) return NAN;
   double lna=log(a);
   if(lna<0 || lna>ln_a[Ntot_a-1]) return NAN;
   return polint3(lna,Ntot_a,ln_a,Z_a); 
}

double ZaN(double a,char*ch) 
{  int n;
   if(sscanf(ch,"%d",&n)!=1) return NAN; 
   if(n<1 || n>Ncdm) return NAN;
   if(Ntot_a==0 || Z_a==NULL) return NAN;
   double lna=log(a);
   if(lna<0 || lna>ln_a[Ntot_a-1]) return NAN;
   return polint3(lna,Ntot_a,ln_a,Z_a+(n-1)*Ntot_a); 
}
...
double ZaNEq(double a,char*ch)
{  
   double T=Ta(a);  
   if(isfinite(T))
   { 
      double Yeq= YdmNEq(T,ch);   
      if(isfinite(Yeq)) return  Yeq*2*M_PI*M_PI/45*pow(T*a,3)*hEff(T); 
   }
   return NAN;   
}
...
{  
   ...
   if(isfinite(z))
   { double T=Ta(a);
     return z/( 2*M_PI*M_PI/45.*hEff(T)*pow(T*a,3));
   }    
   return NAN;
}

double YaN(double a,char*ch)
{
   double T=Ta(a);
   if(isfinite(T)) return ZaN(a,ch)/( 2*M_PI*M_PI/45.*hEff(T)*pow(T*a,3)); else return NAN;
}

double YaEq(double a) 
{  double T=Ta(a);
   if(isfinite(T)) return Yeq(T); else return NAN;
}  

double YaNEq(double a,char*ch)
{ 
   double T=Ta(a);
   if(isfinite(T)) return YdmNEq(T,ch); else return NAN;
}
...
double aT(double T)
{  int im;
   for(im=1;im<Ntot_a;im++) if(T_a[im-1]>T_a[im]) break;
   im--;
   if(T>T_a[im] || T<T_a[Ntot_a-1]) return NAN;
   return  exp(polint3(T,Ntot_a-im, T_a+im,ln_a+im));
}   
...
//printf("lna=%E\n", x);  
  ...
  if(s3==0) { T=0; zeq=0;} else {T=T_s3(s3); zeq=Yeq(T)*pow(s3*a,3); }
//printf(" log(a)=%E  T=%E\n",x,T);
  MassCut=2*Mcdm -T*log(_beps_);
  {  double alpha,vSig;
//     if(T==0) vSig=vSigmaI(Tend,_beps_,&alpha); else vSig=vSigmaI(T,_beps_,&alpha);
     double rho_sm=M_PI*M_PI/30*gEff(T)*pow(T,4);
...
static void Xderiv_a1_odeint(double x, double *Y, double *dYdx)
{
  ...
  if(T<=0) zeq=0; 
  {  double alpha,vSig;
     if(T<=0) {vSig=vSigmaI(Tend,_beps_,&alpha);zeq=0;}  else vSig=vSigmaI(T,_beps_,&alpha);
     vSig*=isWimpFactor;
     double w=0;
     double Gamma;
     if(bStat)
...
//    if(Y[0]<0 ) Y[0]*=-1;   
    ...
    rhoIa3_a[na]=rho_f*pow(a,3); if(rhoIa3_a[na] <0 )  rhoIa3_a[na]=0; //  printf("Negative rhoIa %E  at T=%E  a=%E na=%d\n",rhoIa3_a[na],T,a,na);
//    printf("na=%d rhoIa3_a=%E  Y[0]=%E Y[1]=%E   \n", na, rhoIa3_a[na],Y[0],Y[1]);
    if(T<Tend) break;
  } 
  Ntot_a=na+1;
 ...
 if(assignVal("Q",2*Mcdm)==0) calcMainFunc() ;
  ...
  {  double  first=1;
       ...
       for(na=Ntot_a-1;na>0;na--)
       {
          ...
          double alpha,vSig;
          vSig=vSigmaI(T,_beps_,&alpha)*isWimpFactor;
...
//       if(k==0) printf("Starting point: a=%.3E T=%.3E\n",exp(ln_a[na]),T_a[na]);
       double zt=0;     
       if(na) 
       { double T=T_a[na]; 
         a=exp(ln_a[na]);
...
//na=0;Z_a[0]=0;zt=0;       
       ...
       {  
          ...
          double alpha;
//          printf("na=%d a=%E T=%E zt=%E vSigma=%E %E\n",na,exp(ln_a[na]),T_a[na],zt,vSigmaI(T_a[na],_beps_,&alpha)*3.8937966E8
//                                        , vSigmaA(T_a[na],_fast_,Beps));
          Z_a[na]=zt;
       }
       Zend[k]=Z_a[Ntot_a-1]; 
  }
...
static void Xderiv_aN_odeint(double lna, double *Z, double *dZdlna)
{
  static double  *Q=NULL, *A=NULL;
  static int dim=0;
  int Ncdm1=Ncdm+1;
  if(Ncdm+1>dim) 
  {  dim=Ncdm+1; 
     Q=(double*) realloc(Q,sizeof(double)*Ncdm1*Ncdm1*Ncdm1); 
     A=(double*) realloc(A,sizeof(double)*Ncdm*Ncdm);
  }
  dim=Ncdm+1;
  
  double T=polint3(lna,Ntot_a,ln_a,T_a);
  ...
  {
     dZdlna[i-1]=Q[i];
     for(int j=1;j<=Ncdm;j++) dZdlna[i-1]+=( Q[i+dim*j]+Q[i+dim*dim*j])*Z[j-1]/c;

     for(int j=1;j<=Ncdm;j++) for(int k=1;k<=Ncdm;k++)
     dZdlna[i-1]+=Q[i+dim*j + dim*dim*k]*Z[j-1]*Z[k-1]/c/c;
  }
   
  for(int i=0;i<Ncdm;i++)  dZdlna[i]*=a*a*a*s*s/Ha(a);
         
}


static void Xderiv_aN_stiff(double lna, double*Z, double *F, double h,double*dFdlna,double*dFdZ)
{
  static double  *Q=NULL, *A=NULL;
  static int dim=0;
  int Ncdm1=Ncdm+1;
  ...
  for(int i=1;i<=Ncdm;i++)
  {
     ...
     {  F[i-1]+=( Q[i+dim*j]+Q[i+dim*dim*j])*Z[j-1]/c;
        if(dFdZ) dFdZ[(i-1)*Ncdm+j-1]=(Q[i+dim*j]+Q[i+dim*dim*j])/c;
     }   
     for(int j=1;j<=Ncdm;j++) for(int k=1;k<=Ncdm;k++)
     {  F[i-1]+=Q[i+dim*j + dim*dim*k]*Z[j-1]*Z[k-1]/c/c;     
        if(dFdZ)
        {  dFdZ[(i-1)*Ncdm+j-1]+=Q[i+dim*j + dim*dim*k]*Z[k-1]/c/c;
           dFdZ[(i-1)*Ncdm+j-1]+=Q[i+dim*j + dim*dim*k]*Z[i-1]/c/c;
        }
     }
  }
   ...
   {  
      ...
      for(int n=0; dmDecayTab[n].t[0];n++)
      {
        ...
        double w=dmDecayTab[n].width;
        F[t[0]-1] -= w*Z[t[0]-1]/c;
        for(int k=1; t[k];k++) F[t[k]-1]+=w*Z[t[0]-1]/c;
        if(dFdZ)
        { dFdZ[(t[0]-1)+Ncdm*(t[0]-1)]-=w/c;
           for(int k=1; t[k];k++) dFdZ[(t[0]-1)+Ncdm*(t[k]-1)]+=w/c;
        }
        double dm=McdmN[t[0]];
        for(int k=1; t[k];k++) dm-=McdmN[t[k]];
        w*=T/(2*M_PI*M_PI*s)*exp(-dm/T)*gCDM(t[0],T);
        
        for(int k=1;t[k];k++) w*=Z[t[k]-1]/c/(T/(2*M_PI*M_PI*s)*gCDM(t[k],T));
        F[t[0]-1] += w;
        for(int k=1;t[k];k++) F[t[k]-1] -= w;
        if(dFdZ)
        {
           ...
           for(int l=1;t[l];l++)
          {  double ww=w;
             for(int k=1;t[k];k++) if(k!=l) ww*=Z[t[k]-1]/c;
             dFdZ[(t[l]-1)+Ncdm*(t[0]-1)]+=ww/c;
             for(int k=1;t[k];k++) dFdZ[(t[l]-1)+Ncdm*(t[k]-1)]-=ww/c;
          }
  ...
  for(int i=0;i<Ncdm;i++) F[i]*=c;
  if(dFdZ) for(int i=0;i<Ncdm*Ncdm;i++) dFdZ[i]*=c;
  
  if(dFdlna) 
  { double da=fabs(h);
    if(da>0.1) da=0.1;
    Xderiv_aN_stiff(lna+da, Z, dFdlna, h,NULL,NULL);
    for(int i=0;i<Ncdm;i++) dFdlna[i]=(dFdlna[i]-F[i])/da;
  }  
}
...
double darkOmegaNInfl(double *b, double Beps, int *err)
{
 ...
 _fast_=0;
  ...
  double Tmax=0;
  for(int i=0;i<Ntot_a;i++) if(T_a[i]>Tmax) Tmax=T_a[i];
  
 // getInflDecay(H0, Gamma, Trh, &Tmax,aEnd);
...
//  *Trh=Tmax;
  ...
  printf("initvSigmaTab ok\n");

  int simpsonErr=0;
  for(int i=0;i<nBuildChan;i++) simpsonErr=simpsonErr&allChannels[i].simpsonErr;
  if(err) *err=simpsonErr;
  ...
  double  * Z  =(double*)malloc(sizeof(double)*Ncdm);
  
  double  * Yeq=(double*)malloc(sizeof(double)*Ncdm);
  double  * A  =(double*)malloc(sizeof(double)*Ncdm*Ncdm);
  double  * dZ =(double*)malloc(sizeof(double)*Ncdm);
  double  * Q  =(double*)malloc(sizeof(double)*Ncdm1*Ncdm1*Ncdm1);
  REAL    * A2S=(REAL*)  malloc(sizeof(REAL)*Ncdm*(Ncdm+1)/2);
  REAL    * La2=(REAL*)  malloc(sizeof(REAL)*Ncdm);
  REAL    * R  =(REAL*)  malloc(sizeof(REAL)*Ncdm*Ncdm); 
  for(na=Ntot_a-1;na>0;na--)
  {
     double T=T_a[na];
//printf("na=%d\n",na);      
     ...
     for(int i=0;i<Ncdm;i++) {char ch[5]; sprintf(ch,"%d",i+1); Yeq[i]=YdmNEq(T,ch);}

     for(int i=0;i<Ncdm;i++) for(int j=0;j<Ncdm;j++)
     {  int i1=i+1,j1=j+1;
        A[i*Ncdm+j]=Q[i1+Ncdm1*j1]+Q[i1+Ncdm1*Ncdm1*j1];
        for(int k=1;k<=Ncdm;k++) if(Yeq[k-1])  A[i*Ncdm+j]+=
        Yeq[k-1]*(Q[i1+Ncdm1*j1+Ncdm1*Ncdm1*k]+Q[i1+ Ncdm1*k+Ncdm1*Ncdm1*j1]);
     }
     
     a=exp(ln_a[na]);
     double *Zeq=Yeq;
     double s=2*M_PI*M_PI/45*T*T*T*hEff(T);
     for(int i=0;i<Ncdm;i++) Zeq[i]*=s*a*a*a;
     for(int i=0;i<Ncdm*Ncdm;i++) A[i]*= s/Ha(a)/a; 
     
  
     double dlna=-0.01;
     a*=exp(dlna);
     T=polint3(ln_a[na]+dlna,Ntot_a,ln_a,T_a);
     s=2*M_PI*M_PI/45*T*T*T*hEff(T);
     for(int i=0;i<Ncdm;i++)
     { char ch[5]; sprintf(ch,"%d",i+1);  
       dZ[i]= (YdmNEq(T,ch)*s*pow(a,3)-Zeq[i])/(exp(dlna)-1)/a;   // dZeq/da 
     }

     if(solveLinEq( Ncdm, A, dZ)==0)
     {  
        double r=0; for(int i=0;i<Ncdm;i++) { double r_=fabs(dZ[i]/Zeq[i]); if(r_>r) r=r_; }
        if(r<0.01)
        { 
          if(Ncdm==1) { if(fabs(A[0]*a)<20) continue;}
          else 
          {
             int k=0;
             for(int i=0;i<Ncdm;i++) for(int j=i;j<Ncdm;j++) 
             { A2S[k]=0;
               for(int n=0;n<Ncdm;n++) A2S[k]+=A[n*Ncdm+i]*A[n*Ncdm+j];
               k++;
             }  
             rJacobi(A2S,Ncdm, La2 , R);
             int m;  
             for(m=0;m<Ncdm;m++) if( Fabs(La2[m])<400/a/a) break;     
             if(m<Ncdm) continue;
          } 
          for(int i=0;i<Ncdm;i++) Z[i]=Zeq[i]+dZ[i]; 
          break;  
        }
     } 
     
  }
  printf("Starting point: a=%.3E T=%.3E\n",exp(ln_a[na]),T_a[na]);
  isWimpFactor=1;   
  if(na==0)  for(int i=0;i<Ncdm;i++) Z[i]=0;  
  Z_a=  (double *)realloc(Z_a, Ncdm*Ntot_a*sizeof(double));
  for(int i=0;i<Ncdm*Ntot_a;i++) Z_a[i]=NAN;
  int first=1;
  double h=(ln_a[na+1] - ln_a[na])/2;     
  for(;na<Ntot_a-1 ;na++)
  { 
//printf("na=%d a=%E T=%E Z_a=%E %E\n",na, exp(ln_a[na]), T_a[na],Z[0],Z[1]);
     if(Ncdm==1)
     {
        if(odeint(Z, Ncdm, ln_a[na] , ln_a[na+1] , 1.E-3,  (ln_a[na+1] - ln_a[na])/2 , Xderiv_aN_odeint))
        { printf("problem in solving diff. equation: a=%.3E  T=%.3E\n",exp(ln_a[na+1]), T_a[na+1]);
          if(err) *err=256|simpson_err;
          omega=NAN;
          goto errEnd;
        }
     } else 
     {
       ...
       double ymax=0;
       for(int i=0;i<Ncdm;i++) 
       { if(na==0) { char ch[20]; sprintf(ch,"%d",i+1);  yscal[i]=ZaNEq(exp(ln_a[1]),ch);} else yscal[i]=fabs(Z[i]);
         if(ymax<yscal[i]) ymax=yscal[i];
       }  
       for(int i=0;i<Ncdm;i++) { if(yscal[i]<ymax*1E-5) yscal[i]=ymax*1E-5;}
       
       if(stiff( first , ln_a[na] , ln_a[na+1],  Ncdm , Z, yscal  ,1.E-3, &h , Xderiv_aN_stiff))
       { printf("problem in solving diff. equation  a=%.3E  T=%E\n",exp(ln_a[na+1]),T_a[na+1]);
         omega=NAN;
         break;
       }
       first=0;
     }   
     for(int i=0; i<Ncdm;i++) Z_a[na+1 + i*Ntot_a ]=Z[i];         
  }

  double T=T_a[na];
  a= exp(ln_a[na]); 
  double c=2*M_PI*M_PI/45*hEff(T)*pow(T*a,3);
  Yend=realloc(Yend,Ncdm*sizeof(double));
  for(int i=0;i<Ncdm;i++) Yend[i]=Z_a[na+i*Ntot_a]/c;
  
  omega=0;  
  for(int i=0;i<Ncdm;i++) { fracCDM[i+1]=Yend[i]*McdmN[i+1]; omega+=fracCDM[i+1]; }
  if(omega) for(int i=0;i<Ncdm;i++) fracCDM[i+1]/=omega;
...
static double AExt(double T,double y, double vSigA,double vSigS, double vSig3, double vSig4)
{
  double heff,geff,yeq,yeq_;
  heff=hEff(T);
  geff=gEff(T);
  yeq=Yeq(T);
  yeq_=YeqExt(T);
  
  return MPlanck*pow(2*M_PI*M_PI/45.*heff,2./3.)/sqrt(8*M_PI/3.*M_PI*M_PI/30.*geff) // *(y-yeq) 
        *(( 0.5*vSig3/yeq_ +vSig4*(y+yeq))/yeq_/yeq_ )/3.8937966E8;  
...
static double ds3Ext(double T, double vSigA,double vSigS, double vSig3, double vSig4)
{
  double H=sqrt(8*M_PI/3.*M_PI*M_PI/30.*gEff(T))*T*T/MPlanck;
  double s=2*M_PI*M_PI/45*T*T*T*hEff(T);
  double Y_=Yeq(T);
...
double Y2=Yt;
  ...
  for(int i=0; ;i++)
  {
    ...
    findCsExt( T2,&vSigA,&vSigS,&vSig3,&vSig4);
    double dY=dYExtNew(T2,vSigA,vSigS,vSig3,vSig4); 
    double ds=ds3Ext(T2,vSigA,vSigS,vSig3,vSig4);
    double ddY; 
    XderivLnExt(s3_t,&Yt,&ddY);
 
    if(ddY!=0 || Yt!=0)  // otherwize continue with Y=0        
    {   if(isfinite(ds) && ds < fabs(s3_2-s3_t)/100 ) Yt=Yeq(T2)+dY; else
        { 
          err=odeint(&Yt,1 ,s3_t , s3_2 , 1.E-3, (s3_2-s3_t)/2, &XderivLnExt);
          if(err) Yt=Yeq(T2)+dY; 
       }  
    }  
printf("err=%d\n",err);    
...
//    printf("T=%E Y=%E  ds=%E Delats=%E  err=%d\n", T2,Yt, ds, s3_2-s3_t,err);
    lYtab[Ntab]=log(Yt);
    Ttab[Ntab]=T2;
    Ntab++;
    if(T2==Tend) break;
                               ...
                               , double (*vs_3)(double),double (*vs_4)(double),int *err)
{
  ...
  noStiff(vs_a,vs_s,vs_3,vs_4,&Ts,&dY);
  if(Ts>Mcdm) {if(err) *err=64; printf("Temperature of thermal  equilibrium is too large\n"); return NAN;}
  Tstart=Ts;  
