...
double gVZp[6]={0}, gAZp[6]={0};
double Gamma_q_inv=0;

for(int i=1;i<=5;i++)
{
   char*q=pdg2name(i);
   char*Q=pdg2name(-i);
   if(q==NULL || Q==NULL) continue;

   lVert* ZpQq=getVertex(Q,q,Zpname,NULL); if(!ZpQq) continue;
   double coeff[10];
   getNumCoeff(ZpQq,coeff);

//  for(int k=0;k<ZpQq->nTerms;k++) printf(" %s  ", ZpQq->SymbVert[k]); printf("\n"); 
   for(int k=0;k<ZpQq->nTerms;k++) if( strcmp(ZpQq->SymbVert[k],"G(m3)")==0)   { gVZp[i]=coeff[k]; printf(" gVZp[%d]=%.2E\n",i,gVZp[i]);}     else 
                                  if( strcmp(ZpQq->SymbVert[k],"G5*G(m3)")==0){ gAZp[i]=coeff[k]; printf(" gAZp[%d]=%.2E\n",i,gVZp[i]);}  

   char chan[40];
   sprintf(chan,"%s,%s",Q,q);
   Gamma_q_inv+=findBr(L,chan);
}
...
{  char* name=CDM[i];
   char*aname=antiParticle(name);
   char chan[40];
   sprintf(chan,"%s,%s",aname,name);
   Gamma_q_inv+=findBr(L,chan);
}


char*   name[3]={pdg2name(12), pdg2name(14), pdg2name(16)};
char*  aname[3]={pdg2name(-12),pdg2name(-14),pdg2name(-16)};


for(int i=0;i<3;i++) if(name[i])
  { char channel[40];
...
double jcalc=(SQR(gVZp[2])+SQR(gAZp[2]))*findBr(L,"u,U")+(SQR(gVZp[1])+SQR(gAZp[1]))*findBr(L,"d,D")+(SQR(gVZp[4])+SQR(gAZp[4]))*findBr(L,"c,C")+(SQR(gVZp[3])+SQR(gAZp[3]))*findBr(L,"s,S")+(SQR(gVZp[5])+SQR(gAZp[5]))*findBr(L,"b,B");
