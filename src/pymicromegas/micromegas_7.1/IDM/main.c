...
{ 
  ...
  double Gamma=1E-16, HI=100, Br=0.1, MI=1E10;

Gamma = 1.E-20;
HI    = 100;

  double Tmax,Trh,aEnd,Tfo;
  double b=2*Br/MI;
//b=0;  
//  getInflDecay(HInfl,GammaInfl,&Trh, &Tmax,&aEnd);
  getInflDecayPlus(HsmInfl,HInfl,GammaInfl, alphaInfl,wInfl,&Trh, &Tmax,&aEnd);
  displayPlot("T(a)","a",1,aEnd,1,1,"T(a)", 0,Ta,NULL);
  displayPlot("H(a)","a",1,aEnd,1,1,"H(a)", 0,Ha,NULL);
  displayPlot("rho" ,"a",1,aEnd,1,2,"rhoI", 0,rhoIa, NULL
                                 ,"SM",   0,rhoSMa,NULL);
/*
  displayPlot("rho" ,"a",  1.E13, 2E13,0,2,"rhoI", 0,rhoIa, NULL
                                 ,"SM",   0,rhoSMa,NULL);
...
{ char* expName; 
  ...
  double ddF=DD_factor(AllDDexp, 0.1,Maxwell, &expName);
  printf("DD_factor=%.2E,  Name of experiment =%s\n", ddF,expName);
