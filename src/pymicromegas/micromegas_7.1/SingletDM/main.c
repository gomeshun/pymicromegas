...
#define OMEGA  /* Calculate relic density  */
...
numout*cc24;
double vs24(double T) { return vSigmaCC(T,cc24,1);   }
...
#ifdef OMEGA
{   printf("Calculation of relic Density\n");
...
//   displayPlot("vs","T",2,Tstart,0,1,"vSigma",0,vSigmaMem,NULL);
...
#ifdef SHOWPLOTS
    displayPlot("Y","T", 2,Tstart,0,2,"Y",0, YF,NULL, "Yeq",0,Yeq,NULL); 
#endif
   ...
   { 
      ...
      printf("omegaFiDecay(h)=%.3E\n", omegaFi);
       ...
       toFeebleList(NULL);
       Omega=darkOmegaTR(TR,0,  fast,Beps,&err);
       printf("Omega=%E err=%d\n",Omega,err);
       displayPlot("YF&Yeq","T",Tend, Tstart,1,2,"YF",0,YF,NULL,"Yeq",0,Yeq,NULL);
   }
   if(COSMO==Inflaton)
   { printf("Late Inflaton decay\n");
     double Beps=1.E-5;
     double aEnd,Tmax, Trh=0,Omega;
     ...
     getInflDecay(HInfl,GammaInfl,&Trh, &Tmax,&aEnd);
 
#ifdef SHOWPLOTS  
    displayPlot("T(a)","a",1,aEnd,1,1,"T(a)", 0,Ta,NULL);
    displayPlot("H(a)","a",1,aEnd,1,1,"H(a)", 0,Ha,NULL);
    printf("Trh=%E Tmax=%E \n", Trh,Tmax);
#endif  
  Omega=darkOmegaInfl(BrInfl, Beps,&Tfo, &err);  
  
  printf("Omega=%E Tfo=%E\n",Omega,Tfo);
#ifdef SHOWPLOTS  
  displayPlot("Za","a",1,aEnd,1,2,"Za",0,Za,NULL,"ZEq",0,ZaEq,NULL);
#endif  
}
...
printf("\n==== Calculation of CDM-nucleons amplitudes  =====\n");
  ...
  err=nucleonAmplitudes(CDM[1], pA0,pA5,nA0,nA5);
