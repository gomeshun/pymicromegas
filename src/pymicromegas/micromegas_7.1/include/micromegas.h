...
#include"../CalcHEP_src/c_source/num/include/MOinterface.h"
...
extern int  pNum(const char * name);  /* returns PDG code */
extern double pMass(const char * name); /* returns particle mass */
...
extern double pWidth(const char *name, txtList *L);
...
extern int    findVal(const char * name,double * val);
extern int    assignValW(const char * name, double val);
extern double findValW(const char * name);
extern int    readVar(const char *fname);
...
extern double ZpLimCMS(char * ZpName);
...
extern double  h1eff(double m, double chi, int eta);
extern double  g1eff(double m, double chi, int eta);
extern double  p1eff(double m, double chi, int eta);
extern double  n1eff(double m, double chi, int eta);
...
extern double HInfl;
extern double GammaInfl;
extern double BrInfl;
extern double HsmInfl;
extern double alphaInfl;
extern double wInfl;
...
extern double * constSigmaVpbc;
extern double darkOmega(double *Xf,int Fast, double Beps,int *err);
extern double vSigmaMem(double T);
extern char*ExcludedForNDM;
extern double darkOmega2(double fast, double Beps, int *err);

extern double darkOmegaExtTR(double Tr,double Yr, double (*vs_a)(double), double (*vs_s)(double) 
                                         , double (*vs_3)(double), double (*vs_4)(double)
                                         , int *err );

extern double darkOmegaExt(double *Xf, double (*f0)(double), double (*f1)(double)
                                     , double (*f3)(double), double (*f4)(double),int *err);
extern double vs_aExt(double T); 
extern double vs_sExt(double T);
extern double vs_3Ext(double T); 
extern double vs_4Ext(double T);
...
//  ======== Inflaton ======
extern int getInflDecay(double H0, double Gamma, double *Trh, double *Tmax, double *aEnd);
extern int getInflDecayPlus(double HSM, double HI, double Gamma, double alpha, double omega, double *Trh, double *Tmax, double *aEnd);
//                       functions of scale parameter 'a' 
extern  double Ta(double a);
extern  double aT(double T);     //   aT(Ta(a))=1  if maximal  temperature was reached  at aM < a
extern  double Ha(double a);
extern  double rhoIa(double a);  // inflaton energy density
extern  double rhoSMa(double a); // SM  energy    density

extern  double darkOmegaInfl(double b, double Beps,  double  * Tfo,   int *err);
extern double darkOmegaInflExt(double (*vs_a)(double), double (*vs_s)(double) 
                      , double (*vs_3)(double), double (*vs_4)(double)
                                         , int *err );
...
extern  double YaEq(double a);




extern double darkOmegaNInfl(double *b, double Beps, int *err);
extern double ZaN(double a,char*ch);
extern double ZaNEq(double a,char*ch);
extern double YaN(double a,char*ch);
extern double YaNEq(double a, char*ch);
...
extern void improveCrossSection(long n1,long n2,long n3,long n4, double Pcm, double T,
                                                            double * addr);
...
extern double  C1Prod(numout*cc,double p, double T); 
...
extern  double  PlanckCMB_old(double vSigma, double *Sg, double*Se);
...
extern double SpectdNdE(double E, void * tab);
extern double eSpectdNdE(double E, void * tab);  // E*SpectdNdE
extern void  fillSpect(double (*dNdE)(double ), double Emax, double * SpectAr);

extern void   (*addSpectrum)(double*Spect,double*toAdd);
extern void   addSpectrum_old(double*Spect,double*toAdd);
extern void   addSpectrum_new(double*Spect,double*toAdd);
...
extern void boost_old(double Y, double M0, double mx, double*tab);
extern void boost_new(double Y, double M0, double mx, double*tab);
...
extern  double FSRdNdE(double E, double p,double m, double q, int spin2, int med);
...
extern double atmNuFluxI(int nu,double cs, double E);
...
#define RE_DIM     120    // dimention of recoil energy array
#define RE_START   1.E-2  // energy [keV] corresponding to zero element.
...
extern int  nucleusRecoil1(char *WIMP, double(*fv)(double),
      int A, int Z, double J,
      void (*Sxx)(double,double*,double*,double*),
      double * dNdE);
...
extern double  XENON1T(double M);            
extern double  DS50(double M);
extern double  DS50_noB(double M);
extern double  CRESST_III(double M);
extern double  CRESST_III_SDn(double M);
extern double  PICO60(double M);
extern double  PICO60_SDp(double M);
extern double  XENON1T_SDp(double M);
extern double  XENON1T_SDn(double M);
extern double  PandaX4T(double M);
extern double  PandaXS2(double M);

extern double LZ5T(double M); 
extern double LZ5T_SDn(double M); 
extern double LZ5T_SDp(double M); 

extern double XENON_NT(double M);

extern double Darwin_XLZD(double M); //2404.19524 Fig. 10
extern double Neutrino_FloorXeSI(double M); //2109.03116  Fig. S1 (Xenon)
...
extern int smodels(int Run, int nf,double csMinFb, char*ExcludeBSM, char*fileName, char*version, int wrt);
...
// Sterile neutrinos
extern double darkOmegaNu(double T0, double Tinit, int kL,  double sin22, double Le, double Lm, double Lt, double msKeV);
extern double LasymmS(double T); 
extern double fNuSterile(double eps, void * arg);
