...
{ char name[10];
  ...
  { // Mixing
    ...
    double mh[3]; for(int i=0;i<3;i++) mh[i]=slhaVal("MASS",0.,1,25+i*10);
//  printf("MAsses %E %E %E\n", mh[0],mh[1],mh[2]);  
    int l[3]={0,1,2};
    for(int i=0;i<2;) if(mh[l[i]]>mh[l[i+1]]) { int m=l[i];l[i]=l[i+1];l[i+1]=m; if(i>0)i--;else i++;} else i++; 
//  printf("l= %d %d %d\n", l[0],l[1],l[2]);
    for(i=1;i<=3;i++) for(j=1;j<=3;j++) { sprintf(name,"Zh%d%d",i,j); assignValW(name,slhaVal("NMHMIX",Q,2,l[i-1]+1,j));}
//  for(i=1;i<=3;i++) for(j=1;j<=3;j++) printf("Zh%d%d=%E\n", i,j,slhaVal("NMHMIX",Q,2,l[i-1]+1,j));
