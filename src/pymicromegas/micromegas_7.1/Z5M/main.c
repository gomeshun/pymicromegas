...
#define OMEGA       /*  Calculate Freeze out relic density */
...
{ printf("FreezeIn scenario\n");
  ...
  double TR=1E5;
  double omegaFi,omegaFi1,omegaFi2;  
  int fast=0;
  double Beps=1E-5;
  double Xf;
  double Y[2];
  ...
  toFeebleList(CDM[1]);
  omegaFi1=darkOmegaFi(TR,CDM[1],&err);
  printf("darkOmegaFi(%s)=%E\n", CDM[1], omegaFi1);
  
  omegaFi2=darkOmega(&Xf,fast,Beps,&err);
  printf("darkOmegaFO(%s)=%E\n", CDM[2], omegaFi2);
  fracCDM[1]=omegaFi1/(omegaFi1+omegaFi2);
  fracCDM[2]=omegaFi2/(omegaFi1+omegaFi2);
